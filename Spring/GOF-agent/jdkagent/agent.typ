...
静态代理：我们已有项目的基础上，想在一个类中添加功能。有几种方案：

1.直接修改源码：违背了OCP开闭原则。

2.写子类继承，重写方法：耦合度较高。随着业务多子类多

所以我们要用静态代理：为OrderService接口提供一个代理类。符合OCP开闭原则，同时采用的是关联关系，所以程序的耦合度较低。

但是若业务接口很多，一个接口对应一个代理类，显然也是不合理的，会导致类爆炸。所以要用动态代理：

在动态代理中UserServiceProxy代理类是可以动态生成的。
