...
+ 进程管理
  ...
  - 线程
    ...
    - 线程的实现
      ...
      - 轻量级线程
        - 在内核中来支持用户的线程
      - 用户线程的整个线程调度和管理，操作系统是不直接参与的，由用户级线程库函数来管理，用户级线程的模型，也就类似前面提到的多对一的关系，即多个用户线程对应同一个内核线程，内核线程的模型，也就类似前面提到的一对一的关系，即一个用户线程对应一个内核线程
      - 用户线程的优缺点
        - 优点
        ...
        - 缺点
          ...
          #image("Screenshot_20250917_142837.png")
      ...
      - 轻量级线程的优缺点
        ...
        - 在 LWP 之上也是可以使用用户线程的，那么 LWP 与用户线程的对应关系就有三种：
          - 1：1
            - 一个线程对应一个LWP再对应到一个内核线程，如上图的进程4
              - 优点：实现并行，当一个LWP阻塞，不影响其他LWP
              - 缺点：每一个用户线程产生一个内核线程，创建线程的开销较大
          - N：1
            - 多个用户线程对应一个LWP对应一个内核线程，线程管理在用户空间完成的，操作系统不可知
            - 优点：可以开多个用户线程，上下文切换发生在用户空间，切换效率高
            - 缺点：一个用户线程如果阻塞了，整个线程都会阻塞。在多核CPU无法充分利用CPU
          - M:N 
            - 多个用户线程对应多个LWP,LWP再一一对应到内核线程
            - 优点：大部分线程上下文发生在用户空间，且多个线程又可以充分利用多核CPU的资源
  - 调度
    - 调度时机 
      ...
      - 接下来，说说在单核 CPU 系统中常见的调度算法。
        ...
        - 高响应比优先调度算法
          ...
          - 每次进行进程调度时，先计算「响应比优先级」，然后把「响应比优先级」最高的进程投入运行
           #image("Screenshot_20250918_095526.png")
          - 这是不可预估的。所以，高响应比优先调度算法是「理想型」的调度算法，现实中是实现不了的。
          - 时间片轮转调度算法
