...
- http1.1和2.0区别
  ...
  - HTTP/2.0单个 TCP 连接上多路复用（multiplexing），在一个 TCP 连接里可以同时发送多个请求和响应，多个请求/响应可以同时交错发送，不必排队。解决了 HTTP/1.1 的队头阻塞问题。二进制帧（frame）格式，数据被分解为帧进行传输，更高效，也便于解析。支持 请求头压缩（HPACK），减少冗余信息。支持 服务端推送（Server Push），服务端可以主动发送客户端可能需要的资源，减少等待。头部压缩 + 多路复用，极大减少重复传输的数据量。
  - HTTP/3
