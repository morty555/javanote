...
- java版本变更
  - Java 8
    - 引入 Lambda 表达式和函数式接口（Functional Interface）: 允许 (参数) -> 表达式/语句块 的语法形式。 
...
- 什么是泛型，泛型擦除
  ...
  - Java 的泛型是“伪泛型”。Java 使用的是 Type Erasure（类型擦除） 技术。这意味着：
      - 泛型只在编译期有效
      - 运行期泛型信息被擦除（删除）
      - JVM 根本不知道什么是 List<String> 或 List<Integer>
...
- 说说反射
  ...
  - LambdaMetafactory 是什么？
    - LambdaMetafactory 是 JVM 提供的“动态生成函数对象”的工厂，让 Java 的 lambda 表达式能在运行时被优化成接近普通方法调用的速度。
    - lambda 看似像语法糖，但底层不是匿名类，而是 JVM 动态生成一个 invokedynamic 调用点，再通过LambdaMetafactory 给你生成一个高效的函数对象。替换掉传统反射的 Method.invoke()。
  - 为什么要 LambdaMetafactory？
    - Java 8 前，如果你想：用反射调用方法或把方法当函数传递基本都靠Method.invoke()
...
- string为什么不可变
  ...
  - 常量池优化是设计驱动
    - Java 引入了 字符串常量池（String Pool），目的在于节省内存
...
- JVM的字符串常量池说一说，运行时常量池说一说
  - 字符串常量池
    - 堆（Heap）中独立的 StringTable结构
      - KEY是字符串的内容（即字符序列本身），通过内容计算哈希值（hash = value[].hashCode()）
      - VALUE是堆中实际的 java.lang.String 对象引用（oop 指针）
  ...
  - 运行时常量池 (Runtime Constant Pool)
    - JVM 方法区（元空间）中
    - Class 文件常量池的运行时表现形式（加载后存放在方法区）
    - 字面量
      - 如整数常量 100、字符串字面量 "abc"。
      - 字符串字面量是编译期常量，所以放在运行时常量池用于类加载复用；而字符串哈希code存放的是new的string在堆
      - 哈希缓存， String 与对象实例的字符数组 value强绑定，只能存在 堆对象内部。
    - 符号引用
      - 类和接口的全限定名；
      - 字段的名称和描述符；
      - 方法的名称和描述符。
...
- 编译时多态和运行时多态
  ...
  - 运行时多态（动态多态）
    ...
    ```
    - 子类重写父类方法，通过父类引用指向子类对象时，调用的是 子类重写后的方法。
    - a.speak() 在编译期只知道有这个方法，但具体调用哪个版本在运行时才决定（通过虚方法表动态绑定到 Dog 的实现）。


-  Java 里面有哪些基本数据类型，分别占多少个字节？
  #image("Screenshot_20251116_125302.png")
  - Java 语言规范没有明确规定 boolean 大小
  - 在 数组中：boolean 会被优化成 1 个字节，在 对象中：可能被 JVM 填充对齐，占用 1 字节或更多
...
-  Java 里面 LinkedList 和 ArrayList 的优势分别是什么，各自适用场景有哪些？
  - ArrayList 的优势
    - 随机访问快
    - ArrayList 使用 连续内存（底层是数组），空间利用率高，连续内存，数据紧凑，连续内存的元素在物理内存也通常是紧挨着的，CPU 一次从内存读取会顺便把旁边几个元素也读入缓存行 (Cache Line)。而 LinkedList：每个节点分散在内存不同地方，每次访问 next 节点都可能要去内存找 → 慢得多
    - 遍历快，连续存储+顺序访问，可预读优化。
    - 读多写少的业务数据
  - LinkedList 的优势
    - 头部/中间插入删除快 — O(1)
  ...
  - 从“内存角度”怎么做选型？
    - ArrayList 的内存特点
      - 是连续内存块，对 CPU L1/L2 缓存友好，遍历速度远比链表快。
      - 扩容会创建一个更大的数组并复制，但整体内存是紧凑的。
      - 不会为每个元素额外创建对象，因此：单个元素内存开销最小，遍历效率最高
    - LinkedList 的内存特点
      - 3 个引用（item、next、prev）
      - 对象头（对象本身的元数据）
      - 指针本身占 4～8 字节（JVM 32/64 位决定）
      - 节点对象是离散分布的，导致：
        - 大量内存碎片
        - CPU cache 命中率极差
...
- 100M内存空间 给10G无序数据排序 讲了分片然后用堆实现归并排序 追问归并排序和堆实现的具体逻辑
  - 不能一次性加载进内存排序。必须使用 外部排序（External Merge Sort）
  - 分片排序  
    - 从磁盘顺序读取 100MB 数据到内存
    - 在内存中排序
    - 将排序好的结果写回磁盘（一个临时文件）
...
- 那为什么还要链表
  - 红黑树占用更多内存，红黑树节点大小是普通节点大小的两倍
    - 红黑树节点记录key, value，color,parent,left,right
- 如果链表转为红黑树，还会转回来吗
  - 树节点小于等于6
    - 为什么是6而不是8
...
- hashmap为什么线程不安全
  ...
  - 扩容（resize）时的线程安全问题
    ...
    - JDK 1.8：结构改为数组 + 链表/红黑树，虽然死循环问题修复了，但并发写入仍然会导致数据丢失或覆盖。
      ...
      - 覆盖数据的场景：两个线程同时 put 不同 key，但 hash 到同一个桶
      #image("/assets/Screenshot_20251213_134740.png")
...
- 为什么 HashMap 初始容量是 16，而 Hashtable 是 11？
  - 那时工程经验有限,采用「质数容量 + 取模」的传统 hash 表设计
  - 初始容量是 11（质数）
  - HashMap 重新设计，采用现代 hash 表优化：
    - 容量使用 2 的幂
    - 计算 index 不用 %，而用 hash & (n - 1)
    - 更快,更均匀散列
  - 质数可以打散 hash 分布
...
- ConcurrentHashMap底层
  ...
  - JDK8 ConcurrentHashMap 扩容机制——transfer + 多线程协助扩容（迁移机制）。
    - HashMap/ConcurrentHashMap 扩容的核心步骤：
      ...
      - 迁移完成后替换引用
    - HashMap 扩容是单线程迁移，慢 + 会阻塞。ConcurrentHashMap 采用 多线程协助迁移，因此扩容不会拖垮整个应用。
    - ConcurrentHashMap 扩容触发条件
      - size >= threshold (capacity \* loadFactor)
      - 并且另一个线程还没有在扩容中时，当前线程会设置sizeCtl = -(1 + 当前参与扩容的线程数)
    - transfer 迁移的核心思想
      - 扩容本质是把旧表（table）的某一段（range）迁移到新表（nextTable）。
      - 关键点：多个线程可以同时执行 transfer,但它们处理的是 不同 segment 的桶范围。
...
- hashmap，concurrenthashmap有什么区别
  - 线程安全性
    - 多线程下hasmap可能出现链表丢失、死循环（JDK7）、数据覆盖
  - 并发策略
    - HashMap：没有任何同步机制
...
- 说一说synchronized的实现原理
  -  synchronized 底层是 monitor，代码块 synchronized(obj)：synchronized关键字经过编译之后，会在同步代码块前后分别形成monitorenter和monitorexit字节码指令，在执行monitorenter指令的时候，首先尝试获取对象的锁，如果这个锁没有被锁定或者当前线程已经拥有了那个对象的锁，锁的计数器就加1，在执行monitorexit指令时会将锁的计数器减1，当减为0的时候就释放锁。如果获取对象锁一直失败，那当前线程就要阻塞等待，直到对象锁被另一个线程释放为止。
  ...
  -  锁存在于对象头（Mark Word）中
  - synchronized 的锁升级
  - synchronized 是可重入锁，monitor 维护一个 计数器
...
- 如何用栈实现队列？
  - 两个栈，入栈固定放到一个栈中，出栈先把入栈的元素放到另一个栈，再让另一个栈出栈


- ArrayList 和 LinkedList 有什么区别，添加数据和删除数据有什么区别，底层实现
  - 底层实现方面：arraylist是动态数组 (Object[])，linkedlist是双向链表 (Node{prev, next, item})
  - 内存分布：arraylist是连续内存，linkedlist是不连续内存
  - 随机访问：arraylits是o1。，linkedlist是on
  - 插入/删除：arraylist中间操作会移动大量元素，linkedlist中间操作只改节点指针，不需要移动其他元素
  - 扩容机制：arraylist数组满就扩容 1.5 倍并拷贝，linkedlist不需要扩容
  - 额外内存开销：arraylist只存放对象引用，linkedlist每个节点有两个额外指针（prev / next）
  - 选择 ArrayList 的场景
    - 读 > 写（读多写少）
    - 大量随机访问
    - 索引访问频繁的情况
...
- NIO底层原理
  ...
  - Selector（选择器）
    - 是 多路复用器（Multiplexer），允许一个线程同时监视多个 Channel。
    - 底层依赖操作系统的多路复用机制
...
- MS分片了解吗
  - MS分片即是IP分片
  - 定义：当 IP 数据报大于下一跳链路 MTU 时，将数据报拆分成多个小片发送的过程。
  - IPv4 支持分片，IPv6 不允许中间路由器分片（必须在源端分片）。
  - 分片可能导致性能下降，因为每个分片都需要单独处理和路由。
  - 如果任一分片丢失，整个原始数据报需要重传。
  - 尽量通过 Path MTU Discovery（PMTUD） 避免分片。
...
- 账号密码登录的时候怎么保证传输和存储的安全性
  - 全程使用 HTTPS / TLS
  - 后端永远禁止保存明文密码，对密码做不可逆哈希 + Salt，Salt（盐） 是存放在密码哈希中的一段 随机字符串，用来在存储密码时增加安全性。
    - 行业标准是：
      - bcrypt（最常用）
      - scrypt
      - Argon2（最推荐）
...
- aes两种加密方式了解吗说一下
  - AES-ECB
    ...
    - 每个 16 字节块 独立加密
    - 优点：快，实现简单，可并行处理
    - 缺点（非常重要）：安全性差，相同的明文块 → 相同的密文块，容易被攻击者分析出模式信息
  - AES-CBC
    - 每块明文会与前一块密文 XOR 后再加密
    - 第一块需要一个 随机 IV（初始化向量）
    - 优点：安全性比 ECB 高很多，相同明文生成不同密文（因为有 IV）
    - 不能并行加密（但可并行解密），必须保证 IV 安全随机，必须保证 IV 安全随机
...
- 说说ip地址
  ...
  - 知道了子网掩码，该怎么计算出网络地址和主机地址呢？
    - 将 10.100.122.0 和 255.255.255.0 进行按位与运算，就可以得到网络号
    #image("/assets/Screenshot_20251201_190137.png")
    - 将 255.255.255.0 取反后与IP地址进行进行按位与运算，就可以得到主机号。
  - 那么在寻址的过程中，先匹配到相同的网络号（表示要找到同一个子网），才会去找对应的主机。
  - IP 决定“逻辑上去哪台主机”。以太网靠 MAC 地址决定“物理上发送给哪张网卡”。


- NAT协议如何进行公私网ip映射
  - NAT（Network Address Translation，网络地址转换）协议用于在私有网络与公有网络之间进行 IP 地址转换，从而实现私网主机访问公网，同时节约公网 IP 资源。它的核心就是“公私网 IP 映射”。
  - NAT 有几种常见类型，不同类型的映射方式略有不同：
    - 静态 NAT
      - 一对一映射。私网 IP ↔ 公网 IP 固定对应。
      - 适合内部服务器需要公网固定访问时使用，例如 Web 服务器。
      - 数据包到达 NAT 设备时，只查表就直接替换源/目的 IP。
    - 动态 NAT
      - 多个私网 IP 对应一个公网 IP 池。
      - 当私网主机访问公网时，从公网 IP 池中分配一个临时 IP。
      - 如果 IP 池用完，则无法分配。
    - PAT
      - 多个私网 IP 共享同一个公网 IP，通过端口号区分不同会话。
      - 是家庭路由器常用方式。
      - 映射表不仅记录 IP，还记录端口号。
  - 映射过程
    - 假设私网主机 192.168.1.2 发起对公网服务器 8.8.8.8 的请求
    - 私网主机发送数据包
    - NAT 设备查找映射表，没有现有映射，于是创建新的条目：内网: 192.168.1.2:12345 -> 外网: 203.0.113.1:50001
    - NAT 替换数据包的源 IP 和端口：源 IP: 203.0.113.1 源端口: 50001
    - 公网服务器回应，NAT 设备查表，找到对应私网 IP


- 划分子网解决了什么问题
  - 划分子网（Subnetting）主要是为了解决 IP 地址管理和网络性能优化 的问题。
  - IP 地址浪费问题
    - 原本一个大网络（比如 192.168.0.0/16）可能包含 65536 个地址，但很多情况下只需要几百个或几千个地址。
    - 如果不划分子网，剩余的地址会浪费。划分子网可以根据实际需求把大网络拆成多个小子网，合理分配 IP，提高地址利用率。
  - 广播域过大问题
    - 在一个网络中，广播数据包会发送给同一个网络里的所有主机。
    - 网络规模越大，广播流量越多，可能导致网络拥塞。
    - 划分子网可以将广播域缩小到子网内部，减少广播风暴，提高网络性能。
  - 网络管理与安全问题
    - 大型网络如果不划分子网，管理起来非常困难。子网划分可以按部门、楼层或功能分组
  - 路由优化问题
    - 如果没有子网，路由器只能处理大块的网络路由，路由表可能很大。
    - 划分子网后：路由表可以汇总（CIDR 聚合），只需要知道子网地址和掩码就能路由，减少路由表项，优化路由效率

- 子网直接如何进行连接
  - 子网之间要能 直接通信，必须通过路由（Routing）来实现，因为不同子网属于不同的网络地址段，主机默认不能跨网段直接通信。
  - 通过路由器连接
    - 每个子网有一个 网关（Gateway），通常是子网内的路由器接口 IP。
    - 主机发送数据到另一个子网时，会先发给本子网的网关，由路由器转发到目标子网。
    - 例如：主机A想访问子网B的主机B，主机A发数据到 192.168.2.x → 发给 192.168.1.1（网关），路由器转发到子网B的主机B
  - 通过三层交换机连接
  - 静态路由或默认路由
  - 使用 NAT（网络地址转换）
    - 当两个子网属于不同私有网络，且没有直接路由时，可以通过 NAT 让内部主机访问外部子网。
    - 不推荐用于内部局域网互连，只适合跨公网的访问。

- 静态路由和动态路由区别
  - 静态路由：人为配置，简单可靠，但不灵活，适合小型或固定网络。
  - 动态路由：通过协议自动维护路由表，灵活适应变化，适合大型或复杂网络，但消耗更多资源。
...
- ARP协议介绍
  ...
  - ARP 工作原理
    ...
    - 应答阶段（ARP 响应）
      - 局域网中所有主机都会收到请求；
      - 只有 IP 地址匹配的主机会回应；
      - 该主机会单播回复：“192.168.1.10 的 MAC 地址是 xx:xx:xx:xx:xx:xx”。
      - ARP 响应是单播包（只发给请求者）。
    - 缓存阶段（ARP 缓存）
...
- 介绍一下HTTP协议是个什么东西
  - 它是 Web 浏览器和服务器之间通信的规则，也就是浏览器访问网站时用的“语言”。
  - HTTP 的作用主要用于 客户端（浏览器、APP）向服务器发送请求，获取资源（网页、图片、JSON 数据等）。
  - 服务器根据请求返回相应内容
  - HTTP的特点：
    - 无状态：每次请求都是独立的，服务器不记得上一次的请求。需要用户登录状态时通常用 Cookie/Token 维持。
    - 基于文本：请求和响应都是文本形式（比如请求行、请求头、响应头），便于调试。
    - 基于TCP：HTTP 本身不直接保证可靠传输，它依赖 TCP 来保证数据完整、顺序正确。
    - 面向请求-响应：客户端发请求，服务器返回响应，典型的“问答式”通信。


- http1.1和2.0区别
  - HTTP/1.0是短连接，每个请求都要建立连接
  - HTTP/1.1支持 keep-alive 复用连接也就是长连接。请求顺序执行（串行），容易造成 队头阻塞（Head-of-Line Blocking）。文本格式（Request/Response 都是可读文本）。每个请求都会重复发送完整的 headers，冗余多。
  - HTTP/2.0单个 TCP 连接上多路复用（multiplexing），在一个 TCP 连接里可以同时发送多个请求和响应，多个请求/响应可以同时交错发送，不必排队。避免了 HTTP/1.x 那种一个请求阻塞整个连接的情况。即使 HTTP/2 流是独立的，不同流的数据被 TCP 作为一个连续字节流发送，前面的丢包会导致整个 TCP 连接上的所有流阻塞。所以还是没有解决队头阻塞，因为问题在于TCP。二进制帧（frame）格式，数据被分解为帧进行传输，更高效，也便于解析。支持 请求头压缩（HPACK），减少冗余信息。支持 服务端推送（Server Push），服务端可以主动发送客户端可能需要的资源，减少等待。头部压缩 + 多路复用，极大减少重复传输的数据量。
  - HTTP/3
    - 基于Udp的QUIC协议自实现可靠传输机制，并将每个流的数据包隔离，QUIC 连接上的多个 Stream 之间并没有依赖，都是独立的，也不会有底层协议限制，某个流发生丢包了，只会影响该流，其他流不受影响；因此没有队头阻塞问题
      - 队头阻塞问题
        - 请求级别
          - 请求A的响应比B的响应慢，B要等A响应才能响应
        - 传输级别
          - 数据段1丢失，数据段2,3即使被数据方接收也不能交付给上层
    - 更快的连接建立
...
- 你提到了TCP协议，这个TCP协议是什么？它有什么特点？
  - TCP 是一种 面向连接的、可靠的传输协议，主要作用是在网络中 两个主机之间可靠地传送数据。
    - 面向连接：通信前，需要先建立连接（三次握手）。
    - 可靠传输：保证数据完整、按顺序到达。
  - TCP工作在 OSI 模型的 传输层（TCP/IP 模型中也是传输层），为上层应用（如 HTTP、FTP、SMTP）提供可靠的数据传输服务。
  - TCP的主要特点
    - 面向连接（Connection-oriented）
      - 数据传输前，通信双方必须先建立连接（TCP三次握手）。
      - 传输完成后，要断开连接（四次挥手）。
    - 可靠性传输
      - 数据按序到达：TCP会对数据分段，并加上序号，保证接收端按顺序重组。
    ...
    - 拥塞控制（Congestion Control）
      - 避免网络拥堵导致数据丢失。
      - TCP会根据网络状况调整发送速率（慢启动、拥塞避免等算法）。
    - 面向字节流（Byte Stream）
      - TCP把数据看成连续的字节流，而不是独立的报文。
...
- TCP 连接是逻辑还是物理概念？
  - 逻辑
  - TCP 连接指的是两台设备之间，通过 TCP 协议，为了保证可靠传输而维持的一组 状态信息
  - 这些状态信息包括：
    - 四元组（源 IP、源端口、目标 IP、目标端口）
    - TCP 状态（LISTEN、SYN-SENT、ESTABLISHED、FIN-WAIT 等）
    - 滑动窗口大小
    - 序列号 SEQ
    - ACK 确认号
    - 重传计时器
    - 缓冲区内容
  - 为什么可以证明 TCP 连接是逻辑概念？
    - TCP 连接并不依赖于固定的物理路径，一次 TCP 连接的数据包可能走不同的路由
    - TCP 连接可以断网后重连（路由变化仍保持）你拔掉网线再插上，只要没有超时，TCP 连接仍然维持
    - 一个物理链路可同时跑成千上万 TCP 连接
  - 同一条物理链路上有多个 TCP 连接，怎么区分？
    - TCP 连接由“四元组”唯一标识

- UDP和TCP有什么区别
  - UDP是无连接的，不可靠的（不保证数据到达，也不保证顺序）
  - 面向报文 
  - 没有流量控制和拥塞控制
  - 头部开销小
...
- tcp包的序号是如何定的，比如我有一个很大的包，分成了50份，这50个数据包是如何编号的
  - TCP 会给整个数据流的每个字节编号
  - 每个 TCP 包的 seq 是 本包第一个字节的序号
  - 每个 TCP 包的 ack 是 期望收到的下一个字节序号
  - 举例来说，包1的SEQ值为10000,数据长度为1000,覆盖字节范围就是10000-10999，接收方收到包1（1000字节）回复ACK=11000
...
- 描述一下打开百度首页后发生的网络过程
  ...
  - 通过 DNS 获取到 IP 后，就可以把 HTTP 的传输工作交给操作系统中的协议栈。协议栈的内部分为几个部分，分别承担不同的工作。上下关系是有一定的规则的，上面的部分会向下面的部分委托工作，下面的部分收到委托的工作并执行。应用程序（浏览器）通过调用 Socket 库，来委托协议栈工作。协议栈的上半部分有两块，分别是负责收发数据的 TCP 和 UDP 协议，这两个传输协议会接受应用层的委托执行收发数据的操作。协议栈的下面一半是用 IP 协议控制网络包收发操作，在互联网上传数据时，数据会被切分成一块块的网络包，而将网络包发送给对方的操作就是由 IP 负责的。数据包看了这份指南表示：“原来我需要那么多大佬的协助啊，那我先去找找 TCP 大佬！”
  - 获取MAC地址（唯一标识网卡，只在局域网内有效，不会跨路由传播）：当浏览器得到 IP 地址后，数据传输还需要知道目的主机 MAC 地址，因为应用层下发数据给传输层，TCP 协议会指定源端口号和目的端口号，然后下发给网络层。网络层会将本机地址作为源地址，获取的 IP 地址作为目的地址。接下来有包的序号，这个是为了解决包乱序的问题。还有应该有的是确认号，目的是确认发出去对方是否有收到。如果没有收到就应该重新发送，直到送达，这个是为了解决丢包的问题。接下来还有一些状态位。例如 SYN 是发起一个连接，ACK 是回复，RST 是重新连接，FIN 是结束连接等。TCP 是面向连接的，因而双方要维护连接的状态，这些带状态位的包的发送，会引起双方的状态变更。然后将下发给数据链路层，数据链路层的发送需要加入通信双方的 MAC 地址，本机的 MAC 地址作为源 MAC 地址，目的 MAC 地址需要分情况处理。通过将 IP 地址与本机的子网掩码相结合，可以判断是否与请求主机在同一个子网里，如果在同一个子网里，可以使用 ARP 协议获取到目的主机的 MAC 地址，如果不在一个子网里，那么请求应该转发给网关，由它代为转发，此时同样可以通过 ARP 协议来获取网关的 MAC 地址，此时目的主机的 MAC 地址应该为网关的地址。
  ...
  - TCP 模块在执行连接、收发、断开等各阶段操作时，都需要委托 IP 模块将数据封装成网络包发送给通信对象。在 IP 协议里面需要有源地址 IP 和 目标地址 IP。因为 HTTP 是经过 TCP 传输的，所以在 IP 包头的协议号，要填写为 06（十六进制），表示协议为 TCP。
    - 假设客户端有多个网卡，就会有多个 IP 地址，那 IP 头部的源地址应该选择哪个 IP 呢？
      - 这个时候就需要根据路由表规则，来判断哪一个网卡作为源地址 IP。在 Linux 操作系统，我们可以使用 route -n 命令查看当前系统的路由表。
    - 举例：首先先和第一条目的子网掩码（Genmask）进行 与运算，得到结果为 192.168.10.0，但是第一个条目的 Destination 是 192.168.3.0，两者不一致所以匹配失败。再与第二条目的子网掩码进行 与运算，得到的结果为 192.168.10.0，与第二条目的 Destination 192.168.10.0 匹配成功，所以将使用 eth1 网卡的 IP 地址作为 IP 包头的源地址。
  ...
  - 生成了 IP 头部之后，接下来网络包还需要在 IP 头部的前面加上 MAC 头部。在 MAC 包头里需要发送方 MAC 地址和接收方目标 MAC 地址，用于两点之间的传输。一般在 TCP/IP 通信里，MAC 包头的协议类型只使用0800 ： IP 协议和0806 ： ARP 协议
...
- SYN泛洪攻击
  - SYN攻击，又称 SYN Flood 攻击，是一种常见的 DoS（Denial of Service，拒绝服务）攻击，主要针对 TCP协议的三次握手过程。
  - SYN攻击就是利用这个三次握手的过程
    - 攻击者发送大量 伪造源IP 的 SYN 报文给服务器。
    - 服务器收到后，为每个 SYN 报文分配资源（TCP半连接队列）并发送 SYN-ACK。
    - 因为源IP是伪造的，服务器永远收不到最后的 ACK。
  ...
  - 半连接队列
    ...
    - 当客户端返回 ACK 时，服务器就把这个 TCB 移入 全连接队列，表示连接正式建立。

- DNS劫持和DNS污染
  - DNS劫持是攻击者或中间人通过篡改用户DNS解析请求，强行将域名解析到非真实IP地址的行为。
    - 影响范围局部（单个用户/网络）
    - 攻击位置在用户端、路由器、ISP
    - 目的是为了钓鱼、广告、信息窃取
    - 表现：被引导到恶意网站
  - DNS污染是通过向DNS解析过程中注入错误信息，让域名解析结果“污染”，使用户获取错误IP。
    - 影响范围大规模（ISP或全网）
    - 攻击位置在DNS服务器或网络中间
    - 目的是封锁访问、干扰通信
    - 表现：访问失败或错误IP
...
- 既然有 HTTP 协议，为什么还要有 RPC？
  ...
  - HTTP 和 RPC 有什么区别
    ...
    - 传输的内容
      ...
      #image("/assets/Screenshot_20251201_201519.png")
...
- RPC和常规SDK调用区别
  - 调用位置和执行环境
    - RPC调用的函数在远程服务器上执行。本地客户端只是发起请求，等待远程结果返回。通常需要网络通信
    - 常规 SDK 调用函数在本地进程或本地库中执行。不需要网络，直接调用本地内存中的方法。
  - 通信机制
    - RPC涉及 序列化/反序列化，需要处理 网络传输、超时、重试、失败处理。
    - 常规 SDK 调用直接内存调用，无需序列化。
  - 性能差异
    - RPC相比本地调用慢很多，延迟可能是毫秒级甚至更高。



- 负载均衡模型
  - DNS 负载均衡
    - 原理：通过 DNS 解析将同一个域名解析到不同的服务器 IP。
    - 优点：简单、无需额外硬件。
    - 缺点：DNS 缓存生效，无法实时感知服务器负载。
  - 四层负载均衡（L4 Load Balancing / TCP 层）
    - 工作层级：传输层（TCP/UDP）。
    - 原理：根据源 IP、目标 IP、端口等信息分发请求。
    - 缺点：不能基于 HTTP 内容做智能分发。
  - 七层负载均衡
    - 工作层级：应用层（HTTP/HTTPS）。
    - 原理：根据 URL、Cookie、Header、请求内容做分发。
    - 优点：支持会话保持、内容路由。
    - 缺点：性能略低于 L4，需要解析应用层协议。
...
- 如何限制只有 5 个线程能访问？
  - 使用 Semaphore（信号量）
  - 内部维护一个 许可证数量（比如 5 个）
  - 线程进入某段代码前需要获取许可证，没有许可证就阻塞等待，退出后释放许可证。
...
- 核心线程数5，最大线程数10，无界任务队列，问线程池中最多有几个线程工作  
  - 无界任务队列不会满，所以不会创建非核心线程
...
- 怎么调整老年代阈值？ 
  - 控制对象在 Survivor 区复制多少次后进入老年代。
    - Parallel GC 默认值：15
    - G1、ZGC、Shenandoah 不使用年龄晋升，而是通过动态规则判断
  - 大对象直接进入老年代的阈值
  - 老年代空间占用达到多少时触发 Full GC（并与晋升失败相关）
  - Survivor 区比例
...
- 垃圾收集器
  ...
  - ParNew收集器 (复制算法): 新生代收并行集器，实际上是Serial收集器的多线程版本，在多核CPU环境下有着比Serial更好的表现；
    - 注重响应速度，降低停顿时间，适合交互式应用，在多核情况下减少 STW（Stop The World） 时间，而不是最大化吞吐量。
  - Parallel Scavenge收集器 (复制算法): 新生代并行收集器，追求高吞吐量，高效利用 CPU。吞吐量 = 用户线程时间/(用户线程时间+GC线程时间)，高吞吐量可以高效率的利用CPU时间，尽快完成程序的运算任务，适合后台应用等对交互相应要求不高的场景；动态调整 Eden 区比例、幸存区大小、晋升阈值 来让程序吞吐量最大化。
...
- 零拷贝是什么
  ...
  - 零拷贝实现方式
    - mmap 映射文件
      ...
      #image("Screenshot_20251112_150403.png")
    - sendfile 系统调用（Linux）
      - 常用于文件直接发送到网络套接字。
      - 内核直接从文件缓冲区写入到网卡缓冲区，不需要拷贝到用户空间。  
      - sendfile分两种
        - 普通DMA
          #image("Screenshot_20251112_150432.png")
          - 系统调用：用户进程通过sendfile系统调用发起文件传输请求。这一步需要从用户态切换到内核态。
          - DMA传输数据到内核缓冲区：DMA将磁盘上的数据拷贝到内核缓冲区。
          - 内核拷贝数据到网络缓冲区：内核将内核缓冲区的数据拷贝到网络协议栈的缓冲区。
          -  DMA传输数据到网卡：DMA将网络协议栈的缓冲区数据拷贝到网卡。
          - 系统调用返回：系统调用返回，从内核态切换到用户态。
        - sendfile + DMA Scatter/Gather
          #image("Screenshot_20251112_150525.png")
          - 系统调用：用户进程通过sendfile系统调用发起文件传输请求。这一步需要从用户态切换到内核态。
          - DMA传输数据到内核缓冲区：DMA将磁盘上的数据拷贝到内核缓冲区。
          - DMA根据描述信息传输数据到网卡：DMA根据网络缓冲区中的数据描述信息，直接从内核缓冲区传输数据到网卡。
          - 系统调用返回：系统调用返回，从内核态切换到用户态。
    - 网络协议栈优化
      - 零拷贝也可以用于网络接收数据，例如通过 DMA（直接内存访问）将网卡数据直接写入内核缓冲区，然后直接映射给用户程序。
    #image("Screenshot_20251112_150638.png")
- 什么是DMA
  - DMA（Direct Memory Access，直接内存访问）是一种让外设（如硬盘、网卡、声卡）在不经过 CPU 干预的情况下直接读写内存的技术。它是实现零拷贝和高性能数据传输的重要手段之一。

- 什么是网卡缓冲区
  - 网卡是计算机的网络接口硬件，也就是外设。
  - 网卡内部有自己的 缓冲区（Buffer），用于临时存储发送或接收的数据包。
...
- Java对象在JVM内存中的创建过程，并详细说一下对象头
  - 假设代码：Foo f = new Foo(...);
  - 字节码执行遇到 new 指令；new 指令会在运行时常量池中查找要实例化的类（引用常量），如果类还未解析/加载，会触发类加载（ClassLoader）、验证、准备、解析、初始化等阶段（类初始化 <clinit> 在需要时执行——通常在第一次主动使用时触发）。
  ...
  - 调用构造器
    - 分配并写入 this 引用后，JVM 会执行 <init> 构造方法（先调用父类构造方法，完成字段初始化与构造逻辑）。
...
- new一个对象 jvm底层做了什么
  - 类加载与验证，在创建对象之前，JVM 需要确保类已经被加载和初始化
  - 内存分配
  - 对象初始化
    - 设置默认值
    - 设置对象头
...
-  为什么要打破双亲委派？
  - 为了加载某些用户需要覆盖或者动态生成的类
    - 如果父类加载器已经加载了某个类，你的自定义版本永远不会生效。有些场景必须让用户写的类优先于父加载器的类被加载。
    - 你写了一个和父加载器同名的类，你希望用你写的，而不是用系统类库的
...
- redis的zset的底层数据结构？ziplist的作用？hashtable的作用？跳表的作用？
  ...
  - Redis 自己实现的 dict，比普通哈希表更复杂一些，dict内含有两个数组，一个为当前数组一个为扩容数组，同时它支持 渐进式 rehash（扩容）。当 dict 装满到一定比例后，Redis 不会一次性扩容（那样会卡顿），而是采用 渐进式 rehash：
    - 新建一个更大的哈希表 dictht[1]；
...
- 介绍一下跳表 
  - 最底层（Level 0）是普通有序链表，往上每一层都是下层链表的“抽样版”
  - 查找时从最高层开始横向走，走不动就往下走。这样查找就类似二分：每次跳过一大段节点 → 时间复杂度 O(log n)
  - 跳表的随机高度
    - 根据概率决定是否升高高度，p一般为0.5
  - 插入流程
    - 找到每层该插到哪后面
    - 挨个把 next 指针接上
  - 删除流程
    - 和插入一样，先找到所有前驱，再改 next。

- 压缩列表的连锁更新问题 
  - 压缩链表的连锁更新问题 = 更新一个元素的 score 或 member 可能导致后续节点搬移
  - 因为压缩列表是连续内存存储，每个元素包含包括：前一个元素长度（prev_len），元素编码（字符串/整数等），member，score
  - 而且元素长度可变，member 和 score 都是可变长度的，前一个元素长度字段 (prev_len) 可以是 1 字节或 5 字节，取决于前一个元素长度大小，如果前一个元素长度超过 254 字节，就无法用 1 字节表示，所以采用 5 字节编码
  - 举例：原来 "b" 排在 "a" 和 "c" 之间，score 改成 35 后，应该排在 "c" 之后，因为连续内存存储，移动 "b" 节点，需要把 "c" 后面的所有元素向前或向后搬移，腾出位置，如果 "b" 新的编码长度不同（可能 member 或 score 占用字节变多），还会影响前一个元素长度字段 (prev_len)
...
- 对于一个游戏排名系统，有玩家账号、得分、排名，这三个数据，要求能根据账号查找到得分，根据账号能查到排名，根据排名也能查找到玩家账号。
  - ZSET 
  - ZSET根据账号查得分直接利用HASHMAP实现
  - 根据账号查排名需要先通过HASHMAP利用账号查到分数，再利用分数在跳表中SPAN查找排名
    - SPAN过程：跳表的查询先从最高层开始查 若下一跳不满足条件（也就是下一个节点比要查找的节点大）就降层ZAI再查找所以是LOGN
  - 根据排名查账号就是直接在跳表中查询，因为跳表天然按排名排序，直接二分查找也是LOGN
...
- redis的主从同步是怎么实现的
  ...
  - 全量同步过程
    ...
    - RDB 传输，RDB 文件生成完毕后，主节点将文件通过 socket 发送给从节点。从节点接收 RDB 文件后：
      ...
      - 加载新的 RDB 到内存。
    - 命令传播缓冲
      - 在主节点生成 RDB 期间，如果有新的写命令进来，主机会将这些命令写入一个缓冲区（称为 Replication Buffer 或 backlog buffer）。当 RDB 传输完成后，主节点会将缓冲区的写操作以Aof格式再发给从节点，以保持最终一致。
  - 增量同步
    ...
    - 主节点检查 backlog buffer 是否仍然有该 offset 之后的数据：
      - 有：返回 CONTINUE，仅发送增量命令流（AOF/RESP 格式的写命令）。
...
- redis的过期删除和内存淘汰
  ...
  - 那为什么我不过期立即删除？
    - 在过期 key 比较多的情况下，删除过期 key 可能会占用相当一部分 CPU 时间，在内存不紧张但 CPU 时间紧张的情况下，将 CPU 时间用于删除和当前任务无关的过期键上，无疑会对服务器的响应时间和吞吐量造成影响。所以，立即删除策略对 CPU 不友好。
...
- String 是使用什么存储的?为什么不用 c 语言中的字符串?
  ...
  - 扩容算法  
    - 如果目标长度 < 1MB → 新容量 = 2 \* 目标长度
      - 你现在有一个长度 10 的字符串，执行 append 追加 5 字节：目标长度 = 10 + 5 = 15，小于 1MB → 扩大为：30 字节
    - 如果目标长度 ≥ 1MB → 新容量 = 目标长度 + 1MB。你有一个 1.2MB 的字符串，再追加 100KB：目标长度 ≈ 1.3MB,新容量 ≈ 1.3MB + 1MB = 2.3MB
  - 惰性释放，当你执行字符串缩短的操作内存不会缩小，但是可以主动缩小内存使用MEMORY PURGE
...
- Redisson实现原理
  - 锁数据结构：HASH
  - Redisson 的加锁由一个 Lua 脚本完成
  - 锁失败 → 订阅一个 channel，等待锁释放 → 释放锁时会发布消息 wake up 阻塞线程，再重新执行加锁 Lua 脚本
  - Watchdog 看门狗续期机制
...
- 介绍缓存穿透的解决方案及相关经验。
  ...
  - 解决方案 
    ...
    - 缓存对于 null 不写入，但做短暂的“请求级回退缓存（本地缓存）。也就是只缓存查数据库为 null 的 key，TTL 超短（几秒）。

- 缓存雪崩的解决方案
  - 缓存雪崩就是大量的key同时过期
  - 可以给key增加不同的ttl防止
  - 如果redis直接挂了，其实也类似于缓存雪崩，此时设置随机ttl就没用了
    - 如果直接加锁访问数据库，那性能其实下降太多
    - 可以给redis做集群，防止redis挂的太容易
...
- 如何区分局域网
  - 局域网通常使用 相同的 IP 网段，比如 192.168.1.0/24
  - 在 IP 网络上，局域网内的设备
    - 可以直接互相通信（同一广播域）
    - 不需要经过路由器就可以互相发送数据
  - 如果手机和电脑或者两部设备在同一个 Wi-Fi 网络，连接同一个路由器的WIFI也可能因为不同网段而属于不同子网
...
- io多路复用 SELECT和EPOLL和POLL
  ...
  - epoll底层 
    - 红黑树+就绪链表+等待队列
    - epoll_ctl(ADD) 时，内核会将要监控的文件描述符（fd）加入红黑树；,所有监听 FD 存在红黑树中，增删改查为 O(logN)
  ...
  - poll 相比 select 的改进
    - poll 使用一个可变长的数组 pollfd[]代替select的固定大小的FD集合，数量只受系统内存限制。不再受 FD 数量限制
...
- cookie，session，jwt的区别和共同点。
  - Cookie 是存储载体，Session 是服务端状态机制，JWT 是无状态认证令牌。三者解决的核心问题都是：“你是谁 & 你有没有登录”。
  - COOKIE
    - 是什么
      - 浏览器端保存的一小段数据，本质是 HTTP 协议的一部分，会在 同域请求时自动携带
    - 特点
      - 存在客户端（浏览器）
      - 每次请求都会自动带上
      - 大小限制
    ...
    - 常见用途
      - 保存 sessionId
      - 保存 JWT（很多系统这样做）
      - Cookie 本身不负责“登录态逻辑”，只负责“带数据”
  - Session
    - 是什么
      - 服务端保存的用户会话状态
      - 一般通过 sessionId 与客户端关联
    - 工作流程
      - 用户登录成功
      - 服务端创建 Session（存用户信息）
      - 返回 sessionId 给客户端
      - 客户端用 Cookie 保存 sessionId
      - 后续请求携带 sessionId
      - 服务端根据 sessionId 找 Session
    - 特点
      - 有状态
      - 服务端占内存 / Redis / DB
      - 天然支持踢人，登陆失效，修改权限立即失效
    - 缺点
      - 分布式困难
      - 扩展性较差
  - JWT
...
- 使用jwt，如果希望实现后台过期某一用户登录状态，需要添加什么修改？
  - JWT+REDIS
    - jwt是不存在redis里的
    - redis的key是userid value是version
    - 也是通过修改redis的version,然后和jwt比对发现不一样后踢人
  - JWT+数据库id映射version  
    - 如果踢某个人，就是将数据库id对应的version++，这样从jwt拿出来版本号时发现与数据不匹配，就可以实现踢人的效果
  - 发现version不一样后可以发403报错等，然后前端获得403后移除客户端的token
...
- Springboot启动慢可能是什么原因？
  - 环境层问题
    - 网络环境差或 Maven 依赖下载慢
      - 启动第一次会去尝试加载 SNAPSHOT 等依赖，网络差会导致等待很久。
      - 解决方案：
        - 使用国内源（阿里、华为、中科大）
        - 将依赖改为 release 版本
    - JVM 参数不合理
      - 默认的 JVM 内存太小，GC 频繁，也会拉长启动时间。
    - 服务器磁盘 I/O 很慢
      - Spring Boot 启动时要扫描 jar 包、加载 class，磁盘速度差会导致启动卡顿。
  - Spring Boot 自身导致的慢（框架层）
    - 组件扫描范围太大（最常见）
      - 启动时需要扫描所有 \@Component、\@Service、\@Controller。
      - 如果你的项目是 多模块 或者根包层级太高，会扫描一堆没必要的类。
    - 引入 starter 太多，初始化 Bean 多
    - 配置文件解析慢 / 配置太多
    - 自动配置加载太多
  - 日志与监控导致的慢
  - 其他框架导致的慢
...
- spring springboot springmvc区别？
  - Spring 是基础框架，Spring MVC 是 Web 框架，Spring Boot 是快速构建与整合工具。
  -  Spring（核心框架）
    - 核心解决问题
      - 对象如何创建
      - 对象如何管理
    ...
    - 你用 Spring 能做什么？
      ...
      - 事务管理（Transactional）
      - AOP（日志 / 权限 / 监控）
      - 整合 ORM（MyBatis / Hibernate）
    - Spring 本身 不关心 Web
    - 也不负责启动服务器
  - Spring MVC 是什么？
    - 定位：基于 Spring 的 Web MVC 框架
    - 解决的问题
      - HTTP 请求怎么接收？
        - 由 DispatcherServlet 接收所有 HTTP 请求，DispatcherServlet 统一调度后续流程
      - URL 如何映射到方法？
        - 通过 HandlerMapping 找到对应的 Controller 方法
        - Spring 在启动时会扫描 \@Controller / \@RestController，解析 \@RequestMapping / \@GetMapping
        - Spring MVC 在启动时通过 HandlerMapping 建立 URL 到 Controller 方法的映射关系，请求到来时根据 URL 精确匹配方法。
      - 请求参数怎么绑定？
        - 由 HandlerAdapter + 参数解析器完成参数绑定
      - 返回 JSON / 页面？
        - \@RestController = \@Controller + \@ResponseBody
        - 返回对象 → JSON
        - 由 HttpMessageConverter 转换
    - 核心组件
      ...
      - ViewResolver
  - Spring Boot 是什么？
    - 让 Spring 项目 “开箱即用”，减少配置和依赖地狱
    - Spring Boot 解决了什么痛点？
      - 以前（Spring + Spring MVC）：XML 配一堆，手动配置TOMCAT,手动配 Spring MVC，版本冲突频发
      - Spring Boot：自动配置，内置TOMCAT，SATRTER依赖，约定大于配置
...
- Spring事务注解\@Transactional失效场景
  -  方法调用方式问题（自调用导致失效）
    - 在同一个类内部，一个方法调用另一个被 \@Transactional 注解的方法，事务不起作用。
    - Spring 的事务是通过 AOP代理实现的（默认是基于 JDK 动态代理或 CGLIB）。
    - 事务增强只对 代理对象有效。
  ...
  - 事务只对 RuntimeException 回滚
    ...
    - 如果你想让 checked 异常也回滚怎么办
      - 用 rollbackFor
  -  Bean 未被 Spring 托管
    - Spring AOP 代理只会增强 Spring 容器管理的 Bean。

- 两种动态代理和在两种代理下会出现的\@Trasncational注解失效问题
...
- 静态代理和动态代理的区别 
  - 静态代理是在代码编译阶段就已经确定了代理类的类文件，代理类和被代理类都要提前写好。
    - 代码量大：每个被代理类都需要写一个对应的代理类，如果被代理类很多，代理类也会很多。
    - 灵活性差：修改被代理类的接口，需要同步修改代理类。
  - 动态代理是在运行时创建代理对象，不需要事先写好代理类。
    - 运行时生成代理类：通过反射或字节码生成代理对象。
      - jdk动态代理就是反射
      - CGLIB 动态代理就是字节码生成代理对象。CGLIB 通过 生成被代理类的子类，在子类方法里插入增强逻辑，根本上是在内存中生成字节码，然后加载到 JVM。
    - 灵活性高：一个通用的代理类可以代理多个被代理对象。
    - 减少代码量：不需要为每个类单独写代理类。
...
- Mybatis的二级缓存
  - 一级缓存
    ...
    - 清空时期：
      ...
      - 手动调用 clearCache() 
    - 默认（无事务）情况下：每次 mapper 方法调用 = 新的 SqlSession
    ```java
    userMapper.selectUser(1);  // SqlSession A
    userMapper.updateUser(u);  // SqlSession B

    ```
    - 在事务中（\@Transactional）：整个事务 = 一个 SqlSession
    - 如果sqlsession1对a读取，sqlsession2对b修改，a不得知，从缓存中查也会查脏数据
  - 二级缓存（Global Cache）  
    - 默认关闭
    ...
    - 清空时期：
      ...
      - 手动清除缓存
      - 过期时间或容量策略触发清理  
    - SqlSessionA.commit() or close()才会写入二级缓存
    - 二级缓存是单机缓存，分布式不适用
    - 并且在二级缓存是namespcae缓存，如果不同namespcae进行了操作，二级缓存是不知道的
      - 比如a表和b表连表查询，在mapper1查询一次后有缓存，在mapper2修改了，mapper1不知道，还是用的缓存，有脏数据
...
- mybatis中\#{}与\${}的区别
  - \#{} 会被 MyBatis 转换为 ? 占位符，底层使用 PreparedStatement 绑定变量。安全
    - 例如用户输入1 OR 1=1
    - 用 #{} 时实际绑定的值就是："1 OR 1=1"，不会变成 SQL 语句，而是单纯的字符串。
  - \${} 是 字符串直接替换，不做任何处理。
...
- spring是如何解决循环依赖的？
  ...
  - 注意的是，Spring 注入的是同一个对象引用，而不是副本，因此当A初始化完后，B的A变量也从二级缓存变成一级缓存中的对象了
  - 三级缓存的设计的精髓：
...
- 什么是Aop
  ...
  - Spring AOP 实现机制
    ...
    - 其实两者都是生成一个代理类来实现的，JDK动态代理的代理类通过反射调用原方法，在反射前后调用增强逻辑，cglib则是继承你要代理的类，并在该类上加逻辑


- aop是什么设计模式
  - 代理模式：核心实现方式，动态代理/静态代理为目标对象添加增强逻辑
  - 模板方法：通知逻辑中，前置/后置/环绕方法类似模板方法钩子
  - 装饰器模式
...
- 如果让你不用框架实现spring的aop和IoC你会怎么实现
  - IoC（控制反转）怎么实现？
    - 自己写个 Bean 容器BeanFactory,用 HashMap 存实例，把对象创建权从 new 转到容器。
    - 定义\@Component注解标记哪些类要托管
    - 扫描指定包，把这些类找出来
...
- 注解的实现原理
  - 注解本质 → 一种特殊的标记信息，存储在 class 文件中的元数据（Metadata）
  - 注解的生命周期
    - 	只在源码中存在
    - 	编译进 class，但运行期读取不到
    - 	运行期仍然保留，可通过反射读取
  - 注解是如何被读取的？（反射）
  - 注解本身不做事，做事的是读取注解的人（框架/代码/编译器）
  - 使用 \@Target 控制。编译器会把注解对应的 “作用域” 标记写入 class 文件。\@Target(ElementType.METHOD)表示只能写在方法上，编译器会校验，不合规就报错
...
- 5000w数据查询电话号码后4位，如何优化
  ...
  - 倒排表 / 桶分组
    - 后4位只可能有 10,000 种（0000–9999），平均每个桶约 50,000,000 / 10,000 = 5,000 条记录。可以为每个 last4 建一个桶（表或键）
    - 使用 表分区（partition by list/hash on last4），查询 WHERE last4='1234' 只访问一个分区，I/O 更小。
  -  哈希/映射表（last4 -> 列表）
...
- MySQL三范式
  - 第一范式
    ...
    #image("Screenshot_20251104_170335.png")
  - 第二范式
  ...
  - 第三范式
    ...
    - 主键是 学号，院系名 依赖 院系号，院系号 又依赖 学号→ ，学号 → 院系号 → 院系名 是传递依赖 ，违反3nf
...
- 为什么为什么B+树层数代表着io次数
  - 我们说 B+树的层数 = 查询时的 I/O 次数（或接近），其实是在描述 数据库索引在磁盘中的查找过程，因为 每访问一层节点，就意味着一次磁盘 I/O。
...
- 为什么or需要两边都有索引
  - or逻辑相当于满足条件a+满足条件b的行合并去重
  - 方案一：Index Merge（索引合并）
    - 用 age 索引查一批 rowid
    - 用 name 索引查一批 rowid
    - 合并rowid
    - 前提条件age和name都有索引，否则这个方案直接作废。
  -  方案二：全表扫描
    - 对每一行判断 age=20 OR name='Tom'
  - 如果or有一边没索引，相当于查找到一个条件后还要找出 name='Tom' 的那些行。但是name无索引，只能全表，那既然是全表，就没必要索引查找，直接全表就好
...
- 如何理解一致性 
  - 事务执行前后，数据库必须从一个合法状态变为另一个合法状态。
  - 合法状态的定义由：数据类型、唯一约束、外键约束、检查约束、业务逻辑约束等决定。
  - 事务 不能破坏数据库规则；
  - 事务 执行前后数据必须满足所有约束条件。

- 一致性和另外三个的关系？
  - 一致性 C（Consistency）是最终目标，而 A、I、D 是实现一致性的手段。事务的一致性最终由原子性(A)、隔离性(I)、持久性(D) 共同保证。
  - 原子性（A） → 保证一致性不破坏。如果事务中途失败，但部分执行，会导致数据不一致。
  - 隔离性（I） → 防止并发导致不一致  。多个事务同时执行，如果互相干扰，会出现脏读，不可重复读，幻读等
  - 持久性（D） → 确保一致性不会丢失，事务提交后数据库崩溃，如果数据没写到磁盘，就会回到不一致状态。
...
- 三类存储引擎分别支持哪些索引？
  ...
  - MyISAM —— B+ Tree 索引 + 非聚簇结构
    ...
    - 读操作流程
      ...
      - 访问索引文件（.MYI）
        - MyISAM 的索引通常是 B+ 树。
        - 根据索引查找对应的 记录在数据文件中的偏移地址（row offset）。
      - 访问数据文件（.MYD）
...
- MySQL的索引说一下，整个索引查找的过程
  ...
  - InnoDB 索引的底层结构：B+Tree
    - 每个节点对应一个磁盘页（默认 16KB）
    - 内部节点（非叶子节点）只存储 键值（Key）和子节点指针。
    - 叶子节点存储 完整数据行（聚簇索引） 或 主键引用（辅助索引）。
...
- 索引下推
  - 把部分 WHERE 条件的判断逻辑 下推到 存储引擎层，在扫描索引时就能过滤掉不满足的行，从而减少回表次数。
  - MySQL Server 层把一部分条件（涉及索引列的）下推给存储引擎
  - 没有索引的过滤条件在存储引擎把记录交回给server层处理
  - 存储引擎在扫描索引时，直接判断这些条件；
  - 仅当索引层条件满足时，才去回表取整行。
...
-  MySQL执行select语句的流程，特别说明buffer pool的作用
  - 客户端连接
  - 查询缓存（MySQL 8.0 已移除）
  ...
  - 若数据页不在 Buffer Pool 中（未命中）
    ...
    - 如果 Buffer Pool 已满，触发 LRU（最近最少使用）算法 淘汰旧页。
  - 脏页刷新（Flush）
    - 当数据被修改（例如 UPDATE）后，对应页会被标记为 脏页（dirty page）。
...
- char和varchar的区别？
  - 存储方式不同
    - CHAR 是定长字符串，长度固定，不足部分用空格填充。如果超过固定长度，严格模式下报错，非严格模式下截断
    - VARCHAR 是变长字符串，长度可变，实际存储时只占用实际字符长度 + 1 或 2 字节（用于记录长度）。
  - 存储空间
    - CHAR(n) 总是占用 n 字节（如果 n=10，存储 'abc' 实际占用 10 字节）。
    - VARCHAR(n) 占用实际字符长度 + 1 或 2 字节（如果 n=10，存储 'abc' 实际占用 3 + 1 = 4 字节）。varchar有一个前缀，定义 VARCHAR(n) 且 n ≤ 255 → 1 字节长度前缀，如果 n > 255 → 2 字节长度前缀
  - 性能
    - CHAR 由于定长，读取速度稍快，适合存储长度固定的数据（如国家代码、性别等）。
    - VARCHAR 由于变长，适合存储长度不确定的数据，但在频繁更新时可能导致行迁移，影响性能。
  - 最大长度
    - CHAR 最多可定义为 255 字节。
    - VARCHAR 最多可定义为 65535 字节（但受限于行大小和字符集）。
  - varchar的行溢出  
    - VARCHAR 太长导致行数据超过 InnoDB 页大小时，InnoDB 只把前 768 字节存入行内，其余部分放到溢出页（Overflow Page），行内只保存指向溢出页的指针。
    - 发生行溢出会带来什么开销？
      - 随机 IO 大增
      - Update 操作可能导致更多页分裂
        - 如果更新后内容变大，可能再分裂更多溢出页
      - 索引性能下降
        - 溢出页的内容 不会存入二级索引，只会存前缀，其实只有前 767 字节能参与索引。
      - 事务日志变多
        - 溢出页数据量大，写入 undo log / redo log 的量也会明显变多。
  - MySQL 不会自动扩容 VARCHAR
    - 如果你关闭了 SQL 模式 STRICT_TRANS_TABLES，把字符串截断至最大长度发出告警（warning）
  - CHAR 和 VARCHAR 的最大长度都是固定的
  - char长度不足 n 的部分自动填充空格（不是补 0，而是补 " " 空格）长度超过 n 会被截断（而不是报错，具体看 SQL_MODE）
  - VARCHAR 修改时，会在行尾重新存储新的变长数据
  - char固定长度，varchar是由数据长度决定

- 场景：sql中有一个很大的表几千万行，我在查询的时候想查询第9999页的数据，怎么才能查的快一点？
  - SELECT \* FROM big_table LIMIT 9999, 20;跳过前 9999 行，从第 10000 行开始，返回 20 行数据。
  - 直接 LIMIT 9999, 20 很慢，因为 MySQL 必须扫描并丢弃前面 9999*20 = 199,980 行数据，然后才返回你需要的 20 行业。
  - 为什么不能用二分跳到第 9999 行。因为 MySQL 的索引是“按值”排序的，不是“按行号”排序的。行号与 id（或任意字段）之间没有固定关系，可能中间有删除、插入、碎片化等情况，所以 MySQL 根本不知道“第 N 行对应哪个 id”。数据库根本不知道第 9999 行对应什么值。
  - 你需要找到上一页（9998 页）的最后一个 id，例如 last_id = 199,980。SELECT \* FROM big_table WHERE id > 199980 ORDER BY id LIMIT 20;
    - 你的系统是前端“上一页/下一页”翻页，系统自动知道 last_id
    - 用户指定跳转到“第 9999 页”，就用下面的offset方法
  - 如果你只有 offset可以这样查询：用子查询先快速得到起始 id，再查范围。SELECT \* FROM big_table
    WHERE id >= (
        SELECT id FROM big_table ORDER BY id LIMIT 1 OFFSET 199980
    )ORDER BY id LIMIT 20;
  - 为什么offset比limit快？
    - LIMIT 200000, 20每行都要回表查整行数据，只查索引字段 + OFFSET，扫描索引 200000 次并跳过，完全不需要回表 → 因为 id 在索引叶子节点里就有。快的原因是因为id是索引字段 否则只使用offset还是要回表
  - 如果不是按 id 排序？对排序字段建索引,再使用第一个方法
  


- 如果我有一个查询语句，在groupby里面有条件，我使用groupby的条件建索引会加快查询速度吗
  - 不一定。只有在“分组字段本身参与过滤、排序、分组的计算逻辑”时，给 GROUP BY 里的字段建索引才会加速。
  - 如果只是简单的 GROUP BY 字段，而没有 WHERE 条件过滤，建索引往往提升非常有限。
  - 什么时候会加速
    - GROUP BY 字段本身有索引
    - 你 GROUP BY 的是表达式，但你建了函数索引（MySQL 8+）
    - GROUP BY + WHERE 同时使用索引
  - 什么时候不加速？
    - GROUP BY 字段不是操作原字段，而是表达式，表达式没有建索引
    - 仅 GROUP BY，数据量大时优化很有限，因为最终仍要扫描所有数据。
    - GROUP BY 字段不在 WHERE 之前的最左前缀位置
  - 因为where在groupby前执行，需要符合最左前缀
    - 为什么where在groupby前？
      - SQL 执行顺序：FROM → WHERE → GROUP BY → HAVING → SELECT → ORDER BY → LIMIT
      - WHERE 过滤掉不符合条件的行，减少进入 GROUP BY 的数据量。
      - GROUP BY 在 WHERE 之后执行，基于过滤后的结果进行分组计算。如果先groupby,可能会丢失where过滤的条件
    - groupby一定要聚合函数吗？
      - 如果 SELECT 里的所有列都包含在 GROUP BY 中，则不需要任何聚合函数。
      - 非严格模式下可以带非聚合列，但结果不确定。因为groupby已经合并了行，非聚合列可能来自不同的行，结果不确定。
...
- Mysql死锁产生的情况
  - 不同顺序更新相同表的行
    - 如事务一先update行1等待update行2,事务二update行2等待update行1,两个事务都要修改对方持有的行 → 死锁
  ...
  - 唯一索引冲突导致隐式锁等待
    ...
    ```
    - 如果事务1在提交前又去修改别的行，事务2也去修改那行，就可能出现交叉等待而死锁。
  - 间隙锁导致死锁
    #image("Screenshot_20251113_103744.png")
...
- shardingsphere工作流程
  ...
  - SQL 解析阶段（SQL Parser）
    ...
    - 类似数据库的语法分析过程。
  -  路由阶段（SQL Router）
    - 根据 分片规则（Sharding Rule） 决定 SQL 去哪些库、哪些表。
  - 改写阶段（SQL Rewriter）
...
- 了解elasticsearch吗
  - 如果传统mysql查询上百万级的数据的模糊查询，需要全表遍历非常慢
  - elasticsearch采用倒排索引提升效率
  - 倒排索引
    - 将所有数据进行分词
    - 如一个句子i like study分成三个term
    - 每个句子对应一个文本id,那每个句子的文本id都对应若干个词
    - 这样，可以构建以term为key,文本id为value的键值对结构，就可以知道每个词对应在哪个句子有
    - 但是term的数量也有很多，如果不做处理顺序遍历term是On也很慢，于是将term按字典序排列，这样查找的时候就可以二分查找，提高效率
      - 这样排列好后，term这一列就叫做term dictionary,文本id这一列就叫做posting list
  - 倒排索引问题
    - term dictionary的数量很大，放到内存不现实，只能放到磁盘，但是查磁盘又很慢，如何优化？
      - term index
        - 由于每个term都会有相同的前缀，将相同的前缀合并指向后缀就可以节省空间
        - 因此可以构建一个目录树，每个带有前缀信息的节点存放这些词项在磁盘中的偏移量，也就是指向磁盘中的位置
        - 目录树体积小，适合放到内存中
  - stored fields
    - 由于前面倒排索引存放的是句子的id,我们拿到id之后还需要找到原句子
    - 原句子就需要一个空间去存储，我们把这个空间叫做stored fields，是一个行式存储结构
  - doc value
    - 由于业务中我们经常需要对一些字段进行排序查询，如果每次先把和字段有关的数据读出来再查询就很慢
    - 于是我们采用一种空间换时间的方法，将字段集中存放，这个存放的地方就是doc value,这样排序的时候只需要在doc value中读出来排序即可，doc value是列式结构
  - segement
    - 以上四个结构：倒排索引，term index,sorted fields,doc value组成一个复合文件，是具有搜索功能的最小单元
  - lucene
    - 如果多个文件读写到同一个segement时，要对segement多个文件进行修改，并发时性能受影响
    - 于是我们定义segement在生成后无法修改，如果要写新的文件就新生成segement,这样segement只负责读，新segement负责写，并发性能提高
    - 如果segement数量越来越多，我们查找数据的时候就要并发读segement,保证读效率  
    - segement merging
      - 随着segement越来越多，文件句柄有被耗尽的可能，因此会定期将小segement合并成大segement
    - 到此为止，就构成了一个单机文本检索库lucene
  - 但是lucene并不具备高性能，高可用，高扩展，因此需要优化
    - 高性能优化
      - 如果多个服务同时读lucene肯定会造成资源占用导致没有抢占到的服务等待
      - 我们可以将不同类别的信息分到不同的lucene,每个lucene有独自的index name,这样服务访问不同的信息时可以访问不同的lucene，可以降低单个lucene压力
      - 但是这样单个index name的数据依然可能过多
      - 因此我们可以将单个index name的数据进行分片，每一分片的数据叫做一个shard,每一个shard本质上就是一个独立的lucene库，这样多个读写操作可以分摊到多个shard
    - 高扩展性
      - 随着分片变多，如果多个分片都在同一个机器上，导致单机cpu和内存占用过高
      - 于是我们可以增加机器，每个机器存放一部分分片，这些机器就叫做node
    - 高可用
      - 如果node挂了，那node里所有分片都无法对外提供服务
      - 可以给node增加多个副本，分为primary shard 和replica shard
      - 主分片将数据同步给副本分片，副本分片可以提供读操作，也可以在主分片挂了的时候升级成新的主分片
  - node角色分化
    - 一个node含有的功能很多，如集群管理，存储数据，处理请求，如果其中一个功能的能力不足需要扩容节点，就会将其他的功能也扩容，但是其他功能并不需要这这么强的能力，造成资源浪费
    - 因此可以将功能拆开分到不同的node,每个node负责不同的功能
  - 去中心化
    - 在node间引入魔改后的raft,在节点间互相同步数据，让所有node看到的集群数据状态一致，这样集群内的node就能参与选主过程，还能了解到集群内的其他节点是否挂了
  - ES
    - 以上组件合在一起就是es，他对多个语言提供http接口   
  - 写入流程
    - 当客户端发起请求写入到es时
    - 请求会先发到协调节点，协调节点根据hash路由判断数据应该写入到哪个数据节点的哪个分片，找到主分片并写入
    - 分片底层是lucene,所以最终是将数据写入到lucene库里的segement内
    - 将数据固化为倒排索引，termindex,storedfields以及docvalues多个结构
    - 主分片数据写入后会将数据同步给副本分片
    - 副本分片写完后，主分片会响应给协调节点一个ack
    - 最后协调节点响应客户端写入完成
  - 搜索流程
    - 查询阶段
      - 客户端发起请求到协调节点
      - 协调节点根据indexname的信息可以了解到indexname被分为了几个分片，以及这些分片分散到哪个数据节点
      - 将请求转发到这些数据节点的分片上
      - 分片底层的lucene库并发搜索多个segement
      - 利用每个segement内的倒排索引获得文档id,并结合docvalues获得排序信息
      - 分片将结果聚合返回给协调节点
      - 协调节点对信息进行排序聚合，舍弃大部分不需要的数据
    - 获取阶段
      - 协调节点再次拿着id去请求数据节点里的分片
      - 分片底层的lucene库会从segement内的storedfields读取完整文档内容返回给协调节点
      - 协调节点最终将结果返回给客户端
...
- 索引库和mapping一旦添加不可修改，但是可以添加新的字段
- 文档修改
  - 全量修改
    - 删除旧文档添加新文档
  - 增量修改
...
- 说说Netty
  - Netty 是一个高性能、异步、事件驱动的网络通信框架，用来快速开发高并发的 TCP / UDP / HTTP 等网络应用。
  - Netty 是用来干嘛的？
    - Java 原生网络编程太难、太慢。Netty 封装了底层 NIO，帮你把高并发网络通信这件事“优雅地”做好
  - Netty 能做什么？
    - 写服务器和客户端
  - 构建 RPC 框架
  - Netty 核心能力是什么？
    ...
    - 线程模型优秀
    - 编解码框架
      - Netty 不是只帮你收发字节，它还能解决粘包 / 拆包，自定义协议

- 说说Netty线程模型
  - Netty 基于 Reactor 模式，核心线程只有三类：
    - Boss 线程（接客的）
      - accept 新连接（TCP 三次握手完成后的连接）
      - 不负责：读写数据和业务处理
      - 通常 1 个线程就够，只做一件事：接收连接
    - Worker 线程（干活的）
      - 负责
        - Socket 读 / 写
        - IO 事件（read / write / close）
        - 触发 ChannelPipeline
      - 一个 Worker = 一个 EventLoop
      - 一个 Channel 终生绑定一个 EventLoop
      - 默认线程数：CPU 核数 \* 2
      - 同一个 Channel 上的所有 IO 事件，永远由同一个线程处理,因此天然避免并发问题（无锁）
    - 业务线程（可选）
      - 如果业务逻辑 耗时 / 阻塞，可从worker线程抽出交给 业务线程池
  - Netty 线程模型的 3 种形态
    - 单线程模型
    - 多线程模型（Boss + Worker）
    - 主从多线程模型
  - EventLoop / EventLoopGroup 是什么？
    - EventLoop本质：一个死循环 + 一个线程
    - 功能是处理 IO 事件，执行普通和定时任务
    - EventLoopGroup是EventLoop 的集合，负责创建 EventLoop和给 Channel 分配 EventLoop（轮询）
  - channel是什么
    - Channel = 一个网络连接的抽象。对应一个 TCP 连接或一个 UDP 通道。是操作连接的入口
  - ChannelPipeline 是什么？
    - ChannelPipeline = 责任链 / 流水线。本质是一个 Handler 的双向链表。专门用来：处理入站（read）和出站（write）
    - 当发生 IO 事件时：
      - Worker(EventLoop) 监听到事件
      - 读取数据到 ByteBuf
      - 触发 Pipeline
      - Pipeline 按顺序调用 Handler

  - Netty 为什么这么快？
    - 单线程处理一个 Channel，无锁无并发
    - IO 与业务分离，EventLoop 专注 IO
    - Reactor + 多路复用
    - 内存池 + 零拷贝
...
- IO模型有哪些
  ...
  - 信号驱动 I/O，数据到来时内核发信号通知用户进程，然后再读数据，用户调用 read() 获取数据
    - 用户进程先调用fcntl(fd, F_SETOWN, getpid());fcntl(fd, F_SETFL, O_ASYNC);告诉内核：“这个 socket 可读时，请发一个 SIGIO 信号给我。”
    - 当内核检测到该 socket 上有数据到达时内核给用户进程发送一个 SIGIO 信号。
    - 用户进程的信号处理函数（signal handler）被触发，在 handler 里或之后，用户再调用 read(fd, buf, size) 去真正读数据。
    - 在接收到 SIGIO 信号之后，才会同步等待内核把数据拷贝到用户空间。在调用fcntl(fd, F_SETOWN, getpid());fcntl(fd, F_SETFL, O_ASYNC)到SIGIO 信号之间不等待
...
- 和协程的区别呢？
  ...
  - 为什么需要协程
    - 高并发下线程太慢、太贵、太多、太浪费
    - 协程不会阻塞线程（遇到 I/O 会让出）你让线程睡，CPU也跟着睡了；你让协程睡，线程还在干活。

- 进程数据段
  - “进程的数据段”（Data Segment）是 程序在运行时用于存放已初始化的全局变量与静态变量的一块内存区域。它属于程序的逻辑内存布局的一部分。
  - 当一个程序被加载到内存并开始执行时，操作系统会为它分成几个逻辑区域
    - 代码段（Text Segment）：存放可执行机器指令
    - 数据段（Data Segment）：存放已初始化的全局变量和静态变量
    - BSS 段（BSS Segment）：存放未初始化或初始化为 0 的全局变量、静态变量
    - 堆（Heap）：动态分配内存（malloc/new）
    - 栈（Stack）：函数调用、局部变量
  - 其中 数据段 专门放
    - 全局变量（初始化过）
    - 静态变量（初始化过）
    - 常量字符串（取决于编译器，一般放在只读数据段 .rodata）
...
- 进程怎么同步访问共享资源 
  - 进程锁（文件锁、Flock、fcntl）
  - 一个进程通知另一个进程已经处理完事件（信号量，管道，消息队列，SOCKET）
  - 多个进程读写共享内存

- 主线程传递信息给子线程有哪些方法
  - 创建子线程时，通过构造函数或参数传递。也就是主线程创建子线程时，把参数直接塞进子线程对象或 run() 参数里。
    ```java
    class MyThread extends Thread {
    private String message;
        MyThread(String msg) { this.message = msg; }
        public void run() {
            System.out.println("子线程收到: " + message);
        }
    }
  ...
  - 使用线程安全队列（强烈推荐）
    ...
    ```
  - 使用条件变量 / wait-notify 机制
  - 通过 Future / Promise (任务结果传递 + 控制)，主线程把任务提交到线程池，并向子线程传递任务参数。
    ```java
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.println("主线程：等待子线程结果...");
        
        // 3. 主线程阻塞等待结果
        String result = task.get();

        System.out.println("主线程：收到结果 -> " + result);
    }
}
...
- 多个子线程对用一个变量注意什么
  -  竞态条件（Race Condition）
    - 因为 count++ 不是原子操作，它包含读取 count，+1,写回内存，多个线程进行操作时，可能两个线程同时读取COUNT，此时count=1,然后把COUNT都set为count=2,就会出错
  - 可见性问题（Visibility）
    - 一个线程修改了变量，另一个线程可能看不到最新值。
    - 因为 JVM 有线程私有缓存，每个线程未必读到主存最新值。
  - 指令重排（Reordering）
    - JVM 有可能为了优化而调整指令顺序，多线程情况下可能导致异常行为。
  - 正确的处理方式
    - 使用 volatile（保证可见性，禁止部分重排 ，不保证原子性）
    - 使用 synchronized
    - 使用 AtomicInteger，无锁化实现（CAS） 性能高 
    - 使用 Lock（ReentrantLock）需要更灵活锁控制（可中断、公平锁、限时等待）
    - 使用线程安全的类

- 怎么理解可见性，有什么应用
  - 一个线程对共享变量的修改，对另一个线程来说是否“能及时看到”。因为 每个线程都有自己的工作内存（缓存），JVM 为了加速，会把共享变量从主内存复制到自己的缓存中。
  - 使用 volatile后：每次读取 flag 都从主内存读，每次写 flag 都刷新主内存

- 有一个场景，我有多个子线程，我需要在所有子线程跑完之后在进行下一步操作，应该怎么实现
  - Thread.join()，join() 会阻塞当前线程，直到目标线程执行完毕。
  - CountDownLatch
    ```java
    ...
    // 主线程等待所有 countDown() 执行完
    latch.await();

    System.out.println("所有子线程执行完毕，主线程继续");

    ```
  - ExecutorService + Future（可以获取每个子线程返回值）
  ```java
...
- 用户空间和内核空间
  - 用户空间是普通应用程序运行的地方
    - 权限低：不能直接访问硬件（如磁盘、网卡、内存管理）。
    - 如果崩溃不会影响系统：一个程序挂了不会导致整个系统蓝屏。
  - 内核空间运行操作系统核心代码
    - 进程管理（创建、调度）
    - 内存管理（虚拟内存、页表）
    - 文件系统（读写文件）
...
- 内存管理
  ...
  - 为什么需要内存管理？
    - 物理内存有限
    - 多个进程需要同时运行，需要隔离
    - 提供更大“虚拟”内存 → 利用磁盘进行扩展（swap）
    - 提升访问速度（缓存、TLB、局部性）
  - 内存管理的核心技术
    - 连续分配（已过时）
      - 缺点：外部碎片严重
    - 分页（Paging）—现代系统主流
      - 将内存分成大小固定的 页（Page），通常 4KB。
      - 解决外部碎片
      - 逻辑地址与物理地址无关
      - 依靠 页表（Page Table） 建立映射
        - 访问虚拟地址时：通过页表把虚拟页转换为物理页
        - CPU 有专门的缓存加速： TLB
    - 分段（Segmentation）（保护 + 共享）
      - 内存按逻辑结构分成段：代码段、数据段、堆、栈
      - 每段大小不固定
      - 支持保护（如代码段只读）
    - 段页式（Segmentation + Paging）
      ...
      - 优点：既解决碎片，又支持隔离、保护。
    - 虚拟内存
      -  OS 将不常用的页放到磁盘（swap）使程序认为自己有连续、巨大的内存
      - 按需调页（Demand Paging）
        - 当内存满时选择哪个页换出去
        - 避免一次性加载整个程序
...
- 段式，页式存储展开讲讲
  - 程序天然就被划分为代码段，数据段，堆，栈
  - 段式存储 = 按“程序的逻辑结构”来管理内存
  - 段式逻辑地址格式，逻辑地址 = (段号, 段内偏移)
  - 段表：操作系统为每个进程维护一个段表。
    - 地址转换流程：根据 段号 查段表，判断 偏移 < 段长度（越界检查），物理地址 = 段基址 + 偏移
  - 段式存储的特点
    - 优点
      - 符合程序逻辑结构（语义清晰）
    - 缺点
      - 外部碎片严重
        - 段是“按需分配”的（代码段、数据段、栈段大小不同），段被频繁创建、销毁，导致销毁内存后，空闲内存在段与段之间，这段空闲内存空间有限，无法插入大内存的段
  ...
  - 页式存储的特点
    ...
    - 缺点： 
      - 存在内部碎片
        - 页式内部碎片的本质是“固定大小分配单位”带来的空间浪费。也就是每页大小固定，但是可能并不需要那么多空间
      - 页表可能很大（需要多级页表 / TLB）
  - TLB = 页表的缓存，存在于 CPU 中，命中：1 次访存，未命中：查页表（多次访存）
  - 页表是如何“天生适配”虚拟内存的？
    - 页表项里天然就有「存在位」
      - Present = 0 → 不在内存（但地址合法）
      - Present = 1 → 在内存
    - 分页让「缺页中断」变得自然
    - 页是“固定小粒度”，非常适合换入换出
    - 页替换算法依赖分页结构
...
- TLB
  - TLB 是 CPU 中用于缓存“虚拟页号 → 物理页框号”映射的高速缓存。
  - 如果没有TLB,每次访问一个虚拟地址都要经过很多级页表
    - 为什么要用四级页表
      - 如果只用一个页表存储，页的SIZE太大了
      - 并且进程只用到极小一部分虚拟地址，没必要为“没用到的地址”建页表，它只为“被使用的虚拟地址”按需建立映射。只有当某个虚拟地址区间真的被用到时，才会分配下一层页表。
      - 四级页表的主要内容
        - PML4
          - 作用：顶级页表，指向 PDPT（下一层）
          - 存储内容：下一级 PDPT 的物理页框号（Page Frame Number, PFN）。Present / R/W / U/S / Accessed / Dirty 等控制位。
          - 每个 PML4 条目覆盖 512 GB 的虚拟地址空间
        - PDPT
          - 作用：指向页目录（PD）
          - 存储内容：下一级 PD 的物理页框号，Present / 权限位 / Accessed / Dirty。
          - 每个 PDPT 条目覆盖 1 GB 的虚拟地址空间
        - PD
          - 作用：指向页表（PT）
          - 存储内容：下一级 PT 的物理页框号和Present / 权限位 / Accessed / Dirty
          - 每个 PD 条目覆盖 2 MB 的虚拟地址空间
        -  PT
          - 作用：指向实际物理页
          - 存储内容：物理页框号（PFN）和Present / 权限位 / Accessed / Dirty
          - 每个 PT 条目覆盖 4 KB（页大小）
      - 是的，要获取完整的虚拟地址 → 物理地址映射，必须查到最后一级页表（PT），除非使用大页（Huge Page）。
      - 大页：2MB 或 1GB 页，可以让 PDPT 或 PD 条目直接指向物理页框



  - TLB 的目标把“页表查询”这件事，变成像“寄存器/Cache 一样快”。TLB 本质上就是页表的 Cache


- 不同操作系统的页面置换算法
  - Linux
    - Linux 2.x：Clock（时钟算法）+ 二次机会
      - 使用一个环形队列（clock hand），每个页面有 referenced bit（访问位）
      - 扫描时：R=1 → 清零 R，留着（second chance），R=0 → 换出
    - Linux 发现在真实系统中保持严格 LRU 成本太高，于是采用：
      - Active List（活跃）：最近使用过
      - Inactive List（不活跃）：长期未使用
      - 只从 Inactive List 尾部 换出页面
  -  Windows 
    - Windows NT 系列使用 Working Set（工作集）模型
      - 每个进程都有一个最小 / 最大工作集大小（最近使用的页面集合）
      - 页面置换发生在进程内部（局部置换）
      - 置换时，优先换出不属于任何进程工作集的页面，或在工作集中但最近未被访问的页面


- 分页如何解决碎片问题
  - 什么是外部碎片？
    - 在连续分配方式下（例如分段或早期 OS），内存分配必须找到一个 连续的足够大的空间 才能放下程序。你可能有很多小空隙，但没有一块连续区域够大 ➜ 外部碎片
  - 分页如何避免外部碎片？  
    - 将内存划成固定大小的 页（Page），通常是 4KB。虚拟地址和物理地址都按“页”为单位管理
    - 因此程序不再需要“一整块连续的物理内存”，只要有 若干个分散的物理页 就可以满足需求
    - 程序需要 3 页（共 12KB），OS 找到了这些物理页：[页5][页20][页7]  ← 分散没关系
    - 通过页表建立映射：虚拟页0 → 物理页5 虚拟页1 → 物理页20 虚拟页2 → 物理页7
    - 程序仍然看到的是连续的虚拟内存地址
  - 但仍然存在 内部碎片
    - 一个进程使用 4100 字节，OS 必须分配 2 个 4KB 页（共 8192 字节），剩余 4092 字节浪费 → 内部碎片


- 对比分页和分段
  - 分页是查询和展示层面的策略，用于控制一次返回的数据量；
  - 分段是数据存储和架构层面的设计，用于把大数据集拆小，减少扫描范围。
  - 分页本身并不能降低数据扫描量，尤其是 Offset 分页在大数据量下性能很差；
  - 分段可以从物理或逻辑上缩小查询范围，是解决性能瓶颈的关键手段。
  - 实际工程中通常是先分段，再在分段内做 Keyset 分页。
  - 分页解决的是“我一次取多少数据”的问题，常见实现有 Offset 分页和基于游标的 Keyset 分页；Offset 分页在大 offset 时需要扫描并丢弃大量记录，性能会线性下降。
  - 分段解决的是“数据怎么组织和存放”的问题，例如按时间、ID 范围或哈希对数据进行拆分，能显著减少单次查询的扫描范围和索引大小。
  - 在高并发或大数据量场景下，正确的做法是：先通过分段定位到较小的数据集，再在分段内部使用 Keyset 分页进行查询。

- 分页能不能解决大表慢查询？
  - 不能。分页只是限制返回行数，但 Offset 分页仍然可能扫描大量数据。真正解决大表性能问题的是分段、合理索引或分库分表。


- 上下文切换是什么
  - 上下文切换 = CPU 从执行一个任务，切换去执行另一个任务的过程
  - 上下文切换具体在“切”什么？
    - 线程上下文切换
      - 切换寄存器、栈
      - 不切换地址空间
  ...
  - 上下文切换的目的是什么？
    - 实现多任务并发
      - 单核 CPU 也能“同时”跑多个程序，靠的就是：快速上下文切换 + 时间片轮转
    - 提高 CPU 利用率
      - 如果一个任务等 I/O（磁盘 / 网络）等锁
      - CPU 会保存它的上下文，切换去执行一个就绪态的任务
    - 支持抢占式调度
    - 实现线程/进程隔离与安全
  - 什么时候会发生上下文切换
    - 时间片用完
    - 线程主动 sleep / wait / block
    - I/O 阻塞
    - 系统调用 / 中断
    - 更高优先级任务到来


- 对于一个计算机，有2GB的存储空间，现在使用malloc函数每次申请512MB的空间，最多可以申请几次？
...
- 支付里面有个非常经典的场景——转账。用户的钱减了，商户的钱一定要加。这里要做到强一致性，你觉得应该怎么做？
  ...
  - 分布式事务
      ...
      - Saga 
        ...
        - 协调者驱动（Orchestration）
          ...
          - 如果任意一步失败，协调者调用前面已成功步骤的补偿事务
            - 为什么需要补偿事务
              - 在分布式事务中，一个大事务被拆成多个小的本地事务按顺序执行：如果中间某一步失败，前面的事务已经提交了，系统就处于“半完成状态”，需要回滚。不能直接用数据库回滚（因为本地事务已提交），所以需要设计一个 补偿事务 来撤销之前的操
            - 补偿事务的特性
              - 与原事务逻辑相反
              - 幂等性
              - 局部执行
    ...
    - 可靠消息 + 异步最终一致性
      - 先在本地数据库扣钱，生成 消息（Event / MQ）
      - 消息发送给商户服务，商户服务收到后加钱
      - 如果消息发送失败，重试机制保证最终一致性
      - 高可用，但不是强一致性
    - 行业实践
      - 核心账户系统在单库或强事务数据库中，保证扣钱和加钱的核心操作在同一事务
      - 对于跨库或跨服务场景，用 TCC 或 Saga 模式 处理。
      - 幂等性设计：防止消息重复执行导致多扣。
      - 可靠消息：保证最终一致性，避免资金丢失。


- 分布式自增id算法
  - Snowflake 是一种 基于时间戳生成、有序、全局唯一、高性能 的分布式 ID 生成算法。生成出的 ID 是一个 64 bit 的 long 型整数：
    - 符号位1 bit永远为 0（long 类型正数）
    - 时间戳41 bits记录	当前时间 - 自定义纪元（毫秒）
    - 数据中心 ID 	5 bits，	支持 32 个数据中心
    - 机器 ID	5 bits，每个数据中心 32 台机器
    - 序列号12 bits，每毫秒可生成 4096 个 ID，每秒4096 \* 1000 = 4,096,000 个唯一 ID
  - Snowflake 的核心思想
    - 41-bit 时间戳 确保趋势递增。
    - 机器号 + 数据中心号 保证分布式环境唯一性。
    - 毫秒内序列号递增 解决同一毫秒生成太多 ID 的问题。
    - 无锁、高性能，因为基本是对时间与 bit 操作。
  - 如果每毫秒超过4096怎么办
    - 强制等待下一毫秒再继续生成 ID。
    - 增加序列号位数


- 有一个大文件，内部存储1TB的int32位数据，现在希望能快速判断某一int是否存在（尽可能O（1）），利用什么数据结构？
  - 位图（Bitmap / BitSet）或布隆过滤器（Bloom Filter）
  - 原始数据：1TB，Bitmap：512MB

- 如果能实现判断是否存在，能否实现快速定位？
  - 位图和布隆过滤器无法实现快速定位
  - 快速定位可以使用倒排索引
  - 分段索引
    - 将1tb数据分块
    - 对每一个分块都用一个BITMAP，BITMAP的范围由该分块MIN和MAX决定
    - 这样如果数据集中，可以把数据定位在某一个小块
    - 再对小块进行精确定位
    - 缺点是，如果每一块的数据跨度都大，那时间效率就会很低
