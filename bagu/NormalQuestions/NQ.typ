...
- http1.1和2.0区别
  - HTTP/1.1每个请求/响应都需要单独的 TCP 连接（虽然支持 keep-alive 复用连接）。请求顺序执行（串行），容易造成 队头阻塞（Head-of-Line Blocking）。文本格式（Request/Response 都是可读文本）。每个请求都会重复发送完整的 headers，冗余多。
...
- Redisson实现原理
  ...
  - 每个连接都有Redis 请求队列（待发送）和Redis 响应队列（待解析）
