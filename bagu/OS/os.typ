+ 操作系统面试题
  ...
  - 讲一下段表
    ...
    #image("Screenshot_20251015_143326.png")
  ...
  - 虚拟地址是怎么转化到物理地址的？
    ...
    - 为什么需要TLB
      - 页表查找是非常慢的！通常需要访问 内存 2~4 次（多级页表）。
      - 于是，CPU 在硬件层面引入了一个 TLB（Translation Lookaside Buffer），即 “页表项缓存”。
      - 当 CPU 访问一个虚拟地址时：
        - 首先先查PLB看是否缓存过该虚拟页号的映射；
        - 如果命中（TLB hit） → 直接得到物理页框号；
        - 如果未命中，访问页表查找并更新PLB
  ...
  - 为什么这样安排
    - brk 只能 从堆的顶部连续增长或缩小内存,所有通过 brk 分配的空间都在 同一块连续区域（堆）
      - 特点：
        - 分配速度快
        - 内核代价低
        - 但 只能整体扩展/收缩，不能独立释放其中某一部分 ⇒ 容易 产生内部碎片
    - mmap()：向内核申请独立的虚拟内存区域,每次都是独立的 VMA（virtual memory area）,可以 单独释放（munmap）不影响其他内存区域,开销相对大（需要创建 VMA，页表项等）
  ...
  - 页面置换有哪些算法？
    ...
    - 那其算法目标则是，尽可能减少页面的换入换出的次数，常见的页面置换算法有如下几种：
      ...
      - 时钟页面置换算法
        ...
        - 访问位是操作系统在页表（Page Table）中为每个页面设置的一个标志位，用于记录该页面最近是否被访问过（读或写）。如果是 1：表示最近被访问过，给它一次机会，将访问位置 0，然后移动指针到下一个页面；
      - 最不常用算法
  ...
  - 什么是中断？
    ...
    - 中断分为外部中断和内部中断：
      ...
      - 内部中断分为陷阱、故障、终止：
        ...
        - 故障：故障是在引起故障的指令被执行，但还没有执行结束时，CPU检测到的一类的意外事件。
        - 出错时交由故障处理程序处理，如果能处理修正这个错误，就将控制返回到引起故障的指令即CPU重新执这条指令。如果不能处理就报错。常见的故障为缺页，当CPU引用的虚拟地址对应的物理页不存在时就会发生故障。缺页异常是能够修正的，有着专门的缺页处理程序，它会将缺失的物理页从磁盘中重新调进主存。而后再次执行引起故障的指令时便能够顺利执行了
  ...
  - 你了解过哪些io模型？
    ...
    - 信号驱动I/O模型：应用程序发起I/O操作后，可以继续做其他事情，当I/O操作完成时，操作系统会向应用程序发送信号来通知其完成。适合于需要异步I/O通知的场景，可以提高系统的并发能力。应用程序被信号唤醒后，还需要自己调用 read() 或 write() 才能取出数据。
    - 异步I/O模型：应用程序发起I/O操作后可以立即做其他事情，当I/O操作完成时，应用程序会得到通知。异步I/O模型由操作系统内核完成I/O操作，应用程序只需等待通知即可。适合于需要大量并发连接和高性能的场景，能够减少系统调用次数，提高系统效率。应用程序被通知时，数据已经在用户缓冲区里，可以直接使用，不需要再调用 read()。
  ...
  - 讲一下io多路复用
    - IO多路复用是一种IO的处理方式，指的是复用一个线程，处理多个socket中的事件。能够资源复用，防止创建过多线程导致的上下文切换的开销。
  ...
  - select、poll、epoll 的区别是什么？
    ...
    - epoll
      - 先复习下 epoll 的用法。先用epoll_create 创建一个 epol l对象 epfd，再通过 epoll_ctl 将需要监视的 socket 添加到epfd中，最后调用 epoll_wait 等待数据。
      ...
      - poll 的方式即使监听的 Socket 数量越多的时候，效率不会大幅度降低，能够同时监听的 Socket 的数目也非常的多了，上限就为系统定义的进程打开的最大文件描述符个数。因而，epoll 被称为解决 C10K 问题的利器。
        - C10K问题指的是如何让一台服务器同时处理 1 万个（甚至更多）客户端的连接请求而仍然保持高性能。
  ...
  - redis，nginx，netty 是依赖什么做的这么高性能？
    ...
    - Redis
      ...
      - 从 6.0 开始，Redis 引入了 I/O 多线程 来提升网络性能。新模型结构（多 Reactor + 单命令执行线程）
    - Netty
