+ JAVA并发面试题
  ...
  - syncronized锁升级的过程讲一下 
      ...
      - 偏向锁：这个是在偏向锁开启之后的锁的状态，如果还没有一个线程拿到这个锁的话，这个状态叫做匿名偏向，当一个线程拿到偏向锁的时候，下次想要竞争锁只需要拿线程ID跟MarkWord当中存储的线程ID进行比较，如果线程ID相同则直接获取锁（相当于锁偏向于这个线程），不需要进行CAS操作和将线程挂起的操作。
        如果不相同
        - 第一次冲突：撤销偏向
          - 撤销的过程并不是直接变成重量级锁，而是：先暂停拥有偏向锁的线程（安全点操作，比较耗时）。检查持有偏向锁的线程是否还在使用锁对象。如果没有使用，就撤销偏向
  ...
  -  介绍一下AQS
      ...
      - Sync是AQS的实现。 AQS主要完成的任务：
        ...
        - 线程的阻塞和解除阻塞；
        - 队列的管理。
        #image("Screenshot_20250912_111033.png")
  ...
  -  Threadlocal作用，原理，具体里面存的key value是啥，会有什么问题，如何解决?
      ...
      - 可能存在的问题
        ...
        #image("Screenshot_20250912_113614.png")
        #image("Screenshot_20250912_114026.png")
        #image("Screenshot_20250912_114052.png")
  - 悲观锁和乐观锁的区别？
  ...
  - CAS 有什么缺点？
    - ABA问题：ABA的问题指的是在CAS更新的过程中，当读取到的值是A，然后准备赋值的时候仍然是A，但是实际上有可能A的值被改成了B，然后又被改回了A，这个CAS更新的漏洞就叫做ABA。只是ABA的问题大部分场景下都不影响并发的最终效果。Java中有AtomicStampedReference来解决这个问题，他加入了预期标志和更新后标志两个字段，更新时不光检查值，还要检查当前的标志是否等于预期标志，全部相等的话才会更新。这个标志可以用版本号实现
    - 循环时间长开销大：自旋CAS的方式如果长时间不成功，会给CPU带来很大的开销。
    - 只能保证一个共享变量的原子操作：只对一个共享变量操作可以保证原子性，但是多个则不行，多个可以通过AtomicReference来处理或者使用锁synchronized实现。
    #image("Screenshot_20250912_115030.png")
  -  为什么不能所有的锁都用CAS？
  ...
  -  voliatle关键字有什么作用？
    ...
    - 禁止指令重排序优化。volatile关键字在Java中主要通过内存屏障来禁止特定类型的指令重排序。
      ...
      #image("Screenshot_20250912_152413.png")
      #image("Screenshot_20250912_152442.png")
  -  指令重排序的原理是什么？
    - 在执行程序时，为了提高性能，处理器和编译器常常会对指令进行重排序，但是重排序要满足下面 2 个条件才能进行：
      - 在单线程环境下不能改变程序运行的结果
      - 存在数据依赖关系的不允许重排序。
    - 所以重排序不会对单线程有影响，只会破坏多线程的执行语义。
    - #image("Screenshot_20250912_152621.png")
  - volatile可以保证线程安全吗？
  ...
  - volatile和sychronized比较？
    ...
    #image("Screenshot_20250912_153213.png")
  - 什么是公平锁和非公平锁？
  ...
  -  ReentrantLock是怎么实现公平锁的？
    - 公平锁与非公平锁的 lock() 方法唯一的区别就在于公平锁在获取锁时多了一个限制条件：hasQueuedPredecessors() 为 false，这个方法就是判断在等待队列中是否已经有线程在排队了。
    ...
    - 看它的源码就会发现：调用的就是 nonfairTryAcquire()，表明了是不公平的，和锁本身是否是公平锁无关。综上所述，公平锁就是会按照多个线程申请锁的顺序来获取锁，从而实现公平的特性。
  ...
  - 介绍一下线程池的原理
    ...
    - 线程池分为核心线程池，线程池的最大容量，还有等待任务的队列，提交一个任务，如果核心线程没有满，就创建一个线程，如果满了，就是会加入等待队列，如果等待队列满了，就会增加线程执行新提交的任务，如果达到最大线程数量，如果都达到最大线程数量，就会按照一些丢弃的策略进行处理。
