...
- 设计微信抢红包
  - 发红包
    - 系统生成红包记录和份额信息（普通红包固定金额，拼手气红包用随机算法预分配），并把红包状态缓存到 Redis。
    - 检查红包金额与人数正确性
  - 抢红包
    - 核心抢占逻辑放在 Redis，用 Lua 脚本实现原子领取 
      - 检查红包是否还有剩余
      - 判断该用户是否已抢过
      - 分配一个金额
      - 扣减剩余金额和份数
      - 把领取信息写入 Redis
    - 异步化处理：领取结果立即返回，流水、到通过消息队列（Kafka / RabbitMQ）异步入库；
    - 数据库用乐观锁Version
    - 幂等控制：每个请求带唯一 ID，防止重复执行；
  - 弹性扩展：红包服务和 Redis 集群分片部署，热点红包用一致性哈希做分流。
    - Redis 集群分片
      - 每个 key 都会通过 CRC16 计算落到某个槽位上。Redis 集群会自动把不同 slot 分配给不同节点。
      - 这样不同红包天然分布在不同节点上。
    - 一致性哈希或虚拟分片机制
      - 当一个红包非常“火”，例如只有一个红包但有 10 万人抢，Redis 集群分片也帮不了你，因为它还是落在一个 key上。
      - 于是我们再进一步，用 “逻辑分片（sharding）”
      - 人为把一个红包拆成多个 Redis key
    - 应用服务层也要分布式部署。

- 拼手气红包10元5个人保证公平性怎么实现
  - 算法层面的不公平
    - 如果算法在“抢红包时”动态计算金额（而不是预先生成所有金额），就可能出现顺序不公平。
    - 此时第一个人可选范围最大，后面人金额范围逐渐变窄，先抢的人期望值更大。
    - 正确做法：红包发出时一次性生成所有金额，顺序随机化后存入 Redis。这样顺序就不会影响金额期望。
  - 浮点误差或四舍五入导致的“总额偏差“
  - 系统层面的不公平
    - 抢红包请求顺序 ≠ 抢到顺序
    - 如果没有用 Redis 原子操作，而是请求进入应用层；多线程竞争；后端顺序不确定；就可能出现“先点的人反而没抢到”的情况。
  - 网络延迟或时钟不同步
    - 不按客户端时间排序；
    - 以服务器接收顺序为准。
  - 用户作弊 / 重复请求
    - Redis SETNX(userId) 标记是否已抢；
    - 或通过 Lua 脚本保证 “领取红包 + 标记已抢” 原子执行
  - 热点红包分片不均
    - 如果 Redis 分片做得不好（如哈希映射偏斜），导致热点红包都落在一个实例上，那部分用户请求会超时或失败。


- 在游戏对局中，一场战斗会产生大量的交互数据，把它存入数据库中会带来很大的读写压力，如何对战斗记录这个表进行优化，提升读写性能？除了构建缓存和索引，还有什么方式？
  - 架构层面优化
    - 冷热数据分离
  ...
  - 数据库层面优化
    - 表结构与分表策略
      - 表结构与分表策略
        - 将不常用的大字段（比如战斗日志 JSON、录像文件路径、技能使用明细）拆出来单独存表。
        - 主表保留常用字段（如战斗 ID、玩家 ID、胜负、时间），减少热点字段的 I/O 压力。
      - 水平拆分（按行拆分 / Sharding）
        - 按时间分表：例如按月或按天建表 battle_record_202511。查询近期数据快，历史归档数据不影响性能。
        - 按玩家 ID 或战斗 ID 分表：哈希取模或范围切分，减少单表压力。
      - 分库
        - 高并发系统可以把战斗记录单独放在独立库，避免和其他业务表争资源。          
  - 数据存储优化
    - 战斗日志可以用 JSON 压缩或者二进制序列化（protobuf、MessagePack）。减少磁盘 I/O，提高写入速度。
  - 数据库引擎和配置
    - 选择合适的存储引擎
    - 批量提交和连接池
  - 查询优化
    - 战斗记录日志大时，不要每次查询都 SELECT \*，只取需要字段。
    - 分级缓存


- 玩家的数据用什么持久化的方式来存储更加合适？
  - 对局内改动比较小的信息，直接AOF进行记录，大的间隔之间使用RDB持久化