  ...
  - 内存模型
    ...
    - 对象的访问定位
      ...
      - 句柄
        ...
        #image("Screenshot_20251113_112028.png")
      - 直接指针
        - 如果使用直接指针访问，reference 中存储的直接就是对象的地址。
        #image("Screenshot_20251113_112105.png")
...
public class CacheExample {
  ...
  - 双亲委派模型的作用  
    ...
    - 简化了加载流程：简化了加载流程：通过委派，大部分类能够被正确的类加载器加载，减少了每个加载器需要处理的类的数量，简化了类的加载过程，提高了加载效率。
  - 讲一下类加载过程？
...
- 垃圾回收
  ...
  - 垃圾回收算法有哪些？
    ...
    - 标记-整理算法：复制算法在 GC 之后存活对象较少的情况下效率比较高，但如果存活对象比较多时，会执行较多的复制操作，效率就会下降。而老年代的对象在 GC 之后的存活率就比较高，所以就有人提出了“标记-整理算法”。标记-整理算法的“标记”过程与“标记-清除算法”的标记过程一致，但标记之后不会直接清理。而是将所有存活对象都移动到内存的一端。移动结束后直接清理掉剩余部分。
      #image("Screenshot_20250919_113705.png")
  ...
  - 垃圾回收算法哪些阶段会stop the world?
    ...
    - G1的混合回收过程可以分为标记阶段、清理阶段和复制阶段。
      - 标记阶段停顿分析
        ...
        - 再标记阶段：重新标记那些在并发标记阶段发生变化的对象。该阶段是STW的。因为并发标记阶段是并发的，用户线程可能还在修改引用，于是需要STW再标记发生变化的对象
     #image("Screenshot_20250919_124802.png")
      - 清理阶段停顿分析
  ...
  - minorGC、majorGC、fullGC的区别，什么场景触发full GC
    ...
    - Major GC
      - 作用范围：主要针对老年代进行回收，但不一定只回收老年代。在执行过程中 也会顺带回收新生代和部分元空间
    ...
    - Full GC
      ...
      - 触发条件：
        ...
        - Minor GC（新生代垃圾回收）时，如果存活的对象无法全部放入老年代，或者老年代空间不足以容纳存活的对象，则会触发Full GC，对整个堆内存进行回收。强调Minor GC 晋升失败
  ...
  - 垃圾回收器 CMS 和 G1的区别？
    ...
    - 区别四： 垃圾回收的过程不一样
    #image("Screenshot_20250919_125624.png")
    - 区别五: CMS会产生浮动垃圾
      ...
      #image("Screenshot_20250919_125809.png")
      #image("Screenshot_20250919_125959.png")
  - 什么情况下使用CMS，什么情况使用G1?
    - CMS适用场景：
  ...
  -  G1回收器的特色是什么？
    - G1 的特点：
      - G1最大的特点是引入分区的思路，弱化了分代的概念。以前新生代老年代在内存区域的位置是固定的 而G1是随着REGION的角色而定
