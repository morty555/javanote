+ redis面试题
  ...
  - String 是使用什么存储的?为什么不用 c 语言中的字符串?
    ...
    - 结构中的每个成员变量分别介绍下：
      ...
      #image("Screenshot_20251015_140221.png")
  ...
  - Redis有哪2种持久化方式？分别的优缺点是什么？
    ...
    - AOF和RDB优缺点
      ...
      #image("Screenshot_20250928_001903.png")
  ...
  - Redis主从同步中的增量和完全同步怎么实现？
    - 完全同步
      ...
      - 这里新的写操作指的是同步过程中新写入的数据
      #image("Screenshot_20250928_185818.png")
      - 实现过程：
    ...
    - 增量同步
      ...
      - 那 repl_backlog_buffer 缓冲区是什么时候写入的呢？
        ...
        #image("Screenshot_20250928_191445.png")
  - redis主从和集群可以保证数据一致性吗 ？
    ...
    #image("Screenshot_20250928_192026.png") 
  - 哨兵机制原理是什么？
  ...
  - 哨兵机制的选主节点的算法介绍一下
    - 当redis集群的主节点故障时，Sentinel集群将从剩余的从节点中选举一个新的主节点，有以下步骤：
      ...
      - Sentinel集群选举Leader
        ...
        -quorum：法定人数，指多少个 Sentinel 都认为 master 节点主观下线 (SDOWN) 时，才会把它升级为 客观下线 (ODOWN)
      - Sentinel Leader决定新主节点
        - 当Sentinel集群选举出Sentinel Leader后，由Sentinel Leader从redis从节点中选择一个redis节点作为主节点：
          - 过滤故障的节点
          - 选择优先级slave-priority最小（优先级最高）的从节点作为主节点，如不存在则继续
            - slave-priority 的依据就是从节点配置文件中的参数，数值越小优先级越高，0 表示永不当选。
  ...
  - Redis集群的模式了解吗 优缺点了解吗
    ...
    - 接下来的问题就是，这些哈希槽怎么被映射到具体的 Redis 节点上的呢？有两种方案：
      ...
      #image("Screenshot_20250928_193854.png")
    ...
    - Redis集群模式优点/缺点
      ...
      - 缺点：
        - 部署和维护复杂：Redis集群的部署和维护需要考虑到分片规则、节点的布置、主从配置以及故障处理等多个方面，需要较强的技术支持，增加了节点异常处理的复杂性和成本。 
  ...
  - Redis除了缓存，还有哪些应用?
    ...
    - Redis实现分布式锁
      ...
      - redlock: 为了提高分布式锁的可靠性，Redis作者Antirez提出了RedLock算法，它基于多个独立的Redis实例来实现一个更安全的分布式锁。它的基本原理是客户端尝试在多数（大于半数）Redis实例上同时加锁，只有当在大多数实例上加锁成功时才认为获取锁成功。锁的超时时间应该远小于单个实例的超时时间，以避免死锁。该方式可以通过跨多个节点减少单点故障的影响，提高了锁的可用性和安全性。
      #image("Screenshot_20250929_105736.png")
  - Redis支持并发操作吗？
  ...
  - Redis分布式锁的实现原理？什么场景下用到分布式锁？
    ...
    #image("Screenshot_20250929_110848.png")
    - 而解锁的过程就是将 lock_key 键删除（del lock_key），但不能乱删，要保证执行操作的客户端就是加锁的客户端。所以，解锁的时候，我们要先判断锁的 unique_value 是否为加锁客户端，是的话，才将 lock_key 键删除。
    - 可以看到，解锁是有两个操作，这时就需要 Lua 脚本来保证解锁的原子性，因为 Redis 在执行 Lua 脚本时，可以以原子性的方式执行，保证了锁释放操作的原子性。
    #image("Screenshot_20250929_110922.png")
  -  Redis的大Key问题是什么？
    ...
    #image("Screenshot_20250929_111458.png")

  - Redis中有1000w个key，其中只有一个是bigkey，是否也需要处理这个bigkey
    - 哪怕只有一个 bigkey，只要被命中，就可能让整个实例“卡死几秒”。所以，一个 bigkey 也值得重视和处理。
  - 大Key问题的缺点？
  ...
  - 布隆过滤器原理介绍一下
    ...
    #image("Screenshot_20250929_123238.png")
  - 如何设计秒杀场景处理高并发以及超卖现象？
    - 在数据库层面解决
      - 在查询商品库存时加排他锁
    ...
    - 利用分布式锁+分段缓存
      ...
      - 假设场景：假如你现在商品有100个库存，在redis存放5个库存key
