+ 计算机网络面试题
  ...
  -  TCP三次握手过程说一下？
    ...
    #image("Screenshot_20251006_144459.png")
    - 客户端会随机初始化序号（client_isn），将此序号置于 TCP 首部的「序号」字段中，同时把 SYN 标志位置为 1，表示 SYN 报文。接着把第一个 SYN 报文发送给服务端，表示向服务端发起连接，该报文不包含应用层数据，之后客户端处于 SYN-SENT 状态。
    #image("Screenshot_20251006_144530.png")
    - 服务端收到客户端的 SYN 报文后，首先服务端也随机初始化自己的序号（server_isn），将此序号填入 TCP 首部的「序号」字段中，其次把 TCP 首部的「确认应答号」字段填入 client_isn + 1, 接着把 SYN 和 ACK 标志位置为 1。最后把该报文发给客户端，该报文也不包含应用层数据，之后服务端处于 SYN-RCVD 状态。
    #image("Screenshot_20251006_144613.png")
  ...
  - tcp为什么需要三次握手建立连接？
    ...
    - 原因二：同步双方初始序列号
      ...
      - 四次握手与三次握手
        ...
        #image("Screenshot_20251006_145905.png")
    - 原因三：避免资源浪费 
  ...
  - 三次握手和 accept 是什么关系？ accept 做了哪些事情？
    ...
    #image("Screenshot_20251007_144837.png")
  ...
  - 客户端发送的第一个 SYN 报文，服务器没有收到怎么办？
    ...
    - 达到最大重传次数后，会再等待一段时间，时间为上一次超时时间的 2 倍，如果还是没能收到服务端的第二次握手（SYN-ACK 报文），那么客户端就会断开连接。
    ...
    - 避免 SYN 攻击方式，可以有以下四种方法：
      ...
      - 增大 TCP 半连接队列；
        ...
        - 增大 listen() 函数中的 backlog
          - 这个值定义了全连接队列的期望长度（也就是等待 accept() 的连接数量）。
        - 增大 net.core.somaxconn
          - 限制 全连接队列 的最大长度上限（系统层面）。
      - 开启 tcp_syncookies；
  ...
  - TCP 四次挥手过程说一下？
    ...
    - 客户端经过 2MSL 时间之后，也进入 CLOSE 状态；
      - 为什么要等待 2MSL？
        - 确保最后一个 ACK 能被对方收到，假设客户端发送最后一个 ACK 给服务端确认它的 FIN，如果这个 ACK 在网络中丢失；服务端会认为它的 FIN 没被确认；服务端会重发 FIN。此时，客户端必须还在 TIME_WAIT 状态，这样它还能重新发送一次 ACK。如果客户端立即关闭，服务端重发的 FIN 就没人回应，会造成服务端长时间等待，甚至连接异常结束。
        - 确保旧连接的残留报文不会干扰新连接
          - TCP 连接是通过 四元组 唯一标识的：
          - 如果客户端马上关闭并重新建立一个相同四元组的连接，而旧连接的迟到报文（例如旧的 FIN 或数据包）还在网络中漂流，就可能被错误地当作新连接的数据，引发严重混乱。
      - 为什么是“两倍”MSL？
        - 报文 A 从客户端发出，经过最多 MSL 时间到达服务端；
        - 报文 B（对端重发的 FIN）再经过 MSL 时间返回客户端。
        - 等待 2MSL 确保：
          - 我发送的 ACK 足够时间到达对方；
          - 对方若重发 FIN，我也还能应答；
          - 所有旧连接数据都安全过期。
    #image("Screenshot_20251007_152341.png")
  ...
  - HTTPS ECDHE 握手解析
    ...
    #image("/assets/Screenshot_20251201_194724.png")
    - 认识了离散对数，我们来看看 DH 算法是如何密钥交换的。现假设小红和小明约定使用 DH 算法来交换密钥，那么基于离散对数，小红和小明需要先确定模数和底数作为算法的参数，这两个参数是公开的，用 P 和 G 来代称。
    - 然后小红和小明各自生成一个随机整数作为私钥，双方的私钥要各自严格保管，不能泄漏，小红的私钥用 a 代称，小明的私钥用 b 代称。现在小红和小明双方都有了 P 和 G 以及各自的私钥，于是就可以计算出公钥：
      - 小红计算公钥 A ： A = G^a mod P
      - 小明计算公钥 B ： B = G^b mod P
    ...
    - DHE 算法
      - static DH 算法里有一方的私钥是静态的，也就说每次密钥协商的时候有一方的私钥都是一样的，一般是服务器方固定，即 a 不变，客户端的私钥则是随机生成的。DH 交换密钥时就只有客户端的公钥是变化，而服务端公钥是不变的，那么随着时间延长，黑客就会截获海量的密钥协商过程的数据，因为密钥协商的过程有些数据是公开的，黑客就可以依据这些数据暴力破解出服务器的私钥，然后就可以计算出会话密钥了，于是之前截获的加密数据会被破解，所以 static DH 算法不具备前向安全性。
  ...
  - 断开连接时客户端 FIN 包丢失，服务端的状态是什么？
    ...
    - 后续会发生什么？
      - 当服务端还认为连接有效时：
        - 如果它继续发送数据；客户端已经关闭（没有套接字监听此连接）；客户端内核会返回一个 RST（复位）包；服务端收到 RST 后立即关闭连接
    ...
    - 问题来了，什么场景下服务端会主动断开连接呢？
      - 第一个场景：HTTP 没有使用长连接
        - 我们先来看看 HTTP 长连接（Keep-Alive）机制是怎么开启的。
          - 在 HTTP/1.0 中默认是关闭的，如果浏览器要开启 Keep-Alive，它必须在请求的 header 中添加Connection: Keep-Alive
        ...
        - 在前面我们知道，只要任意一方的 HTTP header 中有 Connection:close 信息，就无法使用 HTTP 长连接机制，这样在完成一次 HTTP 请求/处理后，就会关闭连接。问题来了，这时候是客户端还是服务端主动关闭连接呢？
  ...
  - TCP为什么可靠传输
    ...
    #image("Screenshot_20251007_162336.png")
  ...
  - tcp粘包怎么解决？
    ...
    - 一般有三种方式分包的方式：
      ...
      - TCP 粘包是因为 TCP 是流式协议，没有消息边界。多次 send() 的内容可能在接收端一次 recv() 读到一起。


  - TCP的流量控制 
    - 为什么需要流量控制
      - 因为 TCP 收到数据后要放进 接收缓冲区（receive buffer）。如果发送方疯狂发送，而接收方应用层处理得慢 → 缓冲区被占满 → 接收方无法接更多 → 必须让发送方“慢点发”。
    - 流量控制是 发送方根据接收方窗口 rwnd 调整发送速率。允许的发送量 = min(拥塞窗口 cwnd, 接收窗口 rwnd)
    - 如果接收方缓冲区满了，会通告发送方此时必须停下来，不能再发送数据。
    - 但是：发送方不能永远等 → 有 “窗口探测报文”。因为窗口可能会恢复，如果长时间不发消息，双方会“僵死”。
      - 于是 TCP 会定期发送 1 字节的探测报文（ZWP），接收方回复 ACK + 新窗口大小。如果窗口恢复了（Window>0），发送方继续发。
  ...
  - TCP的拥塞控制介绍一下？
    ...
    - 拥塞控制主要是四个算法：
      - 慢启动
        ...
        - 可以看出慢启动算法，发包的个数是指数性的增长。因为每次收到的ack都是指数增长，1到2到4
        - 那慢启动涨到什么时候是个头呢？
      ...
      - 拥塞发生
        ...
        - 发生快速重传的拥塞发生算法
          ...
          #image("Screenshot_20251007_163954.png")
      ...
      - 快速恢复
        ...
        #image("Screenshot_20251007_164533.png")
        #image("Screenshot_20251007_164636.png")
  ...
  - server a和server b，如何判断两个服务器正常连接？出错怎么办？
    - 保活机制。这个机制的原理是这样的：定义一个时间段，在这个时间段内，如果没有任何连接相关的活动，TCP 保活机制会开始作用，每隔一个时间间隔，发送一个探测报文，该探测报文包含的数据非常少，如果连续几个探测报文都没有得到响应，则认为当前的 TCP 连接已经死亡，系统内核将错误信息通知给上层应用程序。
  ...
  - CSRF攻击是什么？
    ...
    #image("Screenshot_20251008_191330.png")
    - 解决CSRF攻击的方法主要有以下几种：
  ...
  - XSS攻击是什么？
    ...
    #image("Screenshot_20251008_191507.png")
