+ 问题排查
  - CPU 100% 问题怎么排查？
    ...
    - 参数：
      ...
      - PR：优先级
      - NI：nice值（越小优先级越高）
      - VIRT：虚拟内存大小（包含 swap、共享库等）
      - RES：常驻内存大小（实际使用的物理内存）
      - SHR：共享内存大小
      - S：状态：R 运行、S 休眠、D 不可中断、Z 僵尸
      - %CPU：进程CPU占用率
      - %MEM：进程内存占用率
      - TIME+：进程累计使用CPU时间
      - COMMAND：命令名/进程名
...
- 查看每一个CPU核情况的linux命令
  ...
  - 一条sql，执行一千次，有一百次明显慢，排查思路
    - 最高频根因分类
      - 执行计划抖动
        - 同一条 SQL有时走索引，有时全表扫描，慢的那 100 次执行计划不一样
        - 原因：参数值不同（where col = ?）数据分布不均（索引选择性变化）
      - 锁等待
      - Buffer Pool / Page Cache 未命中（冷热数据问题）
      - 连接层问题
      - 并发资源争抢
      - SQL 本身非确定性
    - 标准排查顺序
  ...
  - 如何判断是什么溢出
    - jmap
      - jmap需要指定pid,因此需要使用jps找到在运行的java进程
        - 可以查看对象占用
    - jstat（GC 情况）
      - 如果 GC 频率非常高，说明内存马上满。
    - jstack
    ...
    - jconsole
      - 图形化界面可直接选择
      - 可看到堆内存使用量，线程数量，类个数和cpu占用率
      - 可主动执行GC
      - VM概要可以看到详细JVM信息包括垃圾收集器等
    - 分析 dump 文件
      - 重点 1：查看 “Dominator Tree（支配树）”
        - 找出占内存最大的对象
        - 最大的 Top 1、Top 2 占用
        - 是什么类型（List、Map、byte[]、String…）
        - 哪个类持有它（GC Root → ...）
      - 重点 2：查找 “泄漏嫌疑报告（Leak Suspects）”
        - MAT 会自动提示是否有：
          - 某个集合无限增长
          - 某些对象无法被 GC
          - 引用链太长
      - 重点 3：查看 Threads
        - 如果是线程 OOM，可看到几百甚至上千个线程存在。
  ...
  - 内存溢出了 有什么手段可以立马生成堆转储快照么
    ...
    - 如果程序正在运行，可以手动生成：
      - jmap -dump:live,format=b,file=/path/to/heapdump.hprof <pid>
        - 会生成一个文件
        - 文件内容有
          - 类和其对应的Shallow Size（浅层大小）和Retained Size（保留大小 / 保留集大小）
            - shallowsize只包括对象本身占用的内存大小，不包括它引用的对象。只算对象本身的字段和对象头（header）大小。
            - 而retainedsize的定义是如果这个对象被回收，其可以直接回收的内存总量。即对象本身 + 它唯一持有引用的所有子对象
          - biggestobject
            - 堆中占用 最大内存的单个对象
          - GCroots
          - MergePath
