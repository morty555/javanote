+ Mysql面试题 
  ...
  - 有了undolog为啥还需要redolog呢？
    ...
    #image("Screenshot_20250925_150758.png")
  ...
  - redo log怎么保证持久性的？
    - Redo log是MySQL中用于保证持久性的重要机制之一。它通过以下方式来保证持久性：
      ...
      - Checkpoint机制：MySQL会定期将内存中的数据刷新到磁盘，同时将最新的LSN（Log Sequence Number）记录到磁盘中，这个LSN可以确保redo log中的操作是按顺序执行的。在恢复数据时，系统会根据LSN来确定从哪个位置开始应用redo log。
        - Checkpoint 的作用是把内存中较旧的修改按 LSN 有序地刷回磁盘，并把“已刷到磁盘的最大 LSN”记录下来；在恢复时，系统根据这个 checkpoint LSN 去决定从哪里开始重放 redo 日志，从而既保证一致性又限制恢复工作量。
        #image("Screenshot_20250925_151923.png")
        - 为什么 checkpoint 频率/策略很重要（影响与调优点）
          - 恢复时间（crash recovery）：checkpoint 做得越频繁、redo 未刷区间越小，重启时需要重放的 redo 记录越少，恢复越快
          - IO 峰值 vs 延迟：频繁 checkpoint 会产生更多写盘 IO（短而频繁），但能降低重启重放工作；不频繁则把工作推到后台，但万一发生 crash 恢复时间长。
          - 内核/参数相关：
            - innodb_max_dirty_pages_pct / LWM：决定 Buffer Pool 中允许的脏页比例，超过阈值会触发更激进的刷新（page cleaner）。
              - innodb_max_dirty_pages_pct：InnoDB 试图把脏页比例控制在此阈值之下（当脏页比例高于阈值会触发更激进的刷写行为）。
              - innodb_max_dirty_pages_pct_lwm（LWM）：低水位阈值，脏页比例达到 LWM 时 InnoDB 会提前、温和地开始刷页（pre-flushing），以避免脏页一路涨到 high 水位然后突然大规模刷写造成 IO 峰值。lwm=0 表示禁用“提前刷”行为。
              - MySQL 8.x 默认把 innodb_max_dirty_pages_pct 调高（例如 90），并把 lwm 设为 10，这样既允许较多脏页以换取更高吞吐，又用 LWM 做缓冲减少刷写突发。若你用的是旧版本或想回退到“更激进马上刷”的行为，可以把 max 降到 75 并把 lwm 设为 0。
              - 调优建议（实战）
                - SSD 且写吞吐要求高：max=85~95，lwm=5~15（视 io_capacity 而定）可以提升吞吐并避免瞬时写爆。
                - 传统旋转盘：把 max 设低些（例如 60~75）并把 lwm 设为 0 或小值，避免大量随机写。
                - 要避免刷写突发（生产线上常见）：优先尝试打开 lwm（非 0），再调整 innodb_io_capacity 和 page_cleaners。
            - innodb_page_cleaners：后台 page cleaner 线程数量（负责 flush dirty pages）。控制负责把脏页写回磁盘的后台线程数（默认与 innodb_buffer_pool_instances 相同；多个线程可避免单线程成为瓶颈）。
            - innodb_flush_log_at_trx_commit：控制 redo log buffer 写入/刷盘策略（0/1/2 不同值影响事务提交的持久性）——这与 checkpoint 不同，但直接影响 redo 在磁盘上的可见性与崩溃窗口。决定 redo log（日志）何时写入/刷新到磁盘（0/1/2 三档），直接影响事务提交的持久性：1 最安全，2 把性能和可靠性折中（可能丢失最多 1 秒的事务），0 最激进（也可丢失最近 1 秒数据）。
              - 1（默认）：在每次事务提交时，InnoDB 会把 redo 写入日志文件并 fsync（写+刷盘）到磁盘。这是最强的保证——客户端收到 commit 返回即代表数据在持久介质上（避免崩溃丢失）。推荐用于要求严格持久性的场景（同样也是主库在复制中常用的配置，常和 sync_binlog=1 配合）
              - 2：在每次提交时把 log buffer 写入日志文件（写到文件/操作系统缓存），但不马上 fsync；后台每秒会做一次刷盘（fsync）。因此可能丢失最多约 1 秒的事务（如果发生断电/内核崩溃等会丢失内核页缓存的情况）。在磁盘延迟敏感且可接受少量丢失的场景常用来提高 TPS。
              - 0：InnoDB 只每秒把 log buffer 刷写一次（既不在 commit 时写入也不在 commit 时 fsync）。风险最大——在崩溃时可能丢失最近一秒的数据，且在某些环境下持久性更弱于 2（因为 2 在文件层面会做写入操作，0 则可能连写入也延迟）。仅当能接受短时间数据丢失并且对吞吐要求极高时才考虑。
            - 0和2的区别就是，2会在事务提交的时候刷到操作系统缓存，每秒刷盘，而0只按时间来，每秒记录log并刷一次盘
            - 既然只是每秒刷盘，干嘛还要写 redo log？
              - 顺序写，比刷盘快
              - 有了 redo log，即使数据页还没刷盘，也能在宕机后 重放 redo 来修复。刷盘慢可能在刷盘的时候宕机导致信息丢失
  - 能不能只用binlog不用relo log？
    - 不行，binlog是 server 层的日志，没办法记录哪些脏页还没有刷盘，redolog 是存储引擎层的日志，可以记录哪些脏页还没有刷盘，这样崩溃恢复的时候，就能恢复那些还没有被刷盘的脏页数据。
    #image("Screenshot_20250925_155408.png")
    #image("Screenshot_20250925_155537.png")
    #image("Screenshot_20250925_155700.png")
  - binlog 两阶段提交过程是怎么样的？
    ...
    - 事务的提交过程有两个阶段，就是将 redo log 的写入拆成了两个步骤：prepare 和 commit，中间再穿插写入binlog，具体如下：
      ...
      #image("Screenshot_20250925_155958.png")
  ...
  - mysql 两次写（double write buffer）了解吗？
    ...
    - 避免在刷盘的时候出错，页信息会存储 Checksum（校验和）检测数据页是否在写入过程中损坏（比如 torn page，或者磁盘/内存 bit 翻转）。页写入磁盘前，InnoDB 会计算整页内容的 校验和，写到页头（或页尾）。页从磁盘读入内存时，再计算一次校验和。如果和页头记录的不一致，就说明页损坏，需要从 doublewrite buffer 或备份恢复。
  ...
  - 一条sql语句执行过程
    - 因此一次mysql更新语句的过程：先对语句做语法，语义校验构建语法树，然后执行器得到sql语句后查询数据，看数据在缓存里有没有，如果有就返回，没有就在B+树里查询，有索引用索引，得到旧数据后，将更新语句和旧数据一起交给innodb层，innodb层先将旧数据存储到undologbuffer为了回滚，同时redolog记录undologbuffer页的改变，接着innodb开始更新数据，用WAL技术先更新到redologbuffer,同时把binlog更新到缓存binlog cache，在事务提交后采用两阶段提交，将XID写入redolog,把redolog刷到磁盘，并标记为prepare状态，再将XID写入binlog,将binlog刷到磁盘，标记redolog为commit.将binlog刷入磁盘的过程也有缓存binlog cache → binlog 文件缓冲区（内存，OS Page Cache）的过程。然后才将操作系统缓存的binlog写入到磁盘。具体过程按照0,1,2三层策略决定。在这两次数据页写到数据文件之前，要用mysql两次写，先写到doublewritebuffer中，再写到doublewrite磁盘中，最后才刷盘写到数据文件
    + SQL 层解析和优化
      - 语句进入 Server 层，经过 语法/语义分析 → 生成语法树。
      - 优化器选择执行计划（是否走索引、表扫描等）。
      - 执行器开始逐行执行，调用 InnoDB 引擎接口取数据。
    + InnoDB 获取数据
      - 执行器要更新的数据 → 交给 InnoDB。
      - InnoDB 去 Buffer Pool 查是否已有该页：
        - 有 → 直接取旧数据页。
        - 没有 → 从磁盘按 页（16KB） 读入 Buffer Pool。
    + 事务日志准备
      - Undo Log（逻辑日志）：
        - 记录旧值，用于事务回滚和 MVCC。
        - 写入 Undo Log Buffer（在事务提交前可不落盘）。
      - Redo Log（物理日志，WAL 技术）：
        - 记录页修改的物理变化。
        - 先写入 Redo Log Buffer。
    + 两阶段提交（保证 binlog 与 redolog 一致性）
      - 写 Redo Log (prepare 状态)
        - InnoDB 把这次事务的修改写入 Redo Log Buffer，并刷盘（fsync），标记为 prepare。
        - 这时即使宕机，InnoDB 还能根据 Redo Log 找回“已完成但未提交”的数据。
      - 写 Binlog
        - Server 层生成 Binlog event（逻辑操作日志）。
        - 把 binlog 刷盘。
      - Redo Log commit
        - InnoDB 再写一次 Redo Log（把事务 XID 标记为 commit）。
        - 这样保证 binlog 与 redolog 状态一致。
    + 数据写盘与 Doublewrite
      - 修改的数据页在 Buffer Pool 中，后台 Flush 或 Checkpoint 时会写盘。
      - 为了避免部分页写导致页损坏（只写了一半 16KB），InnoDB 使用 Doublewrite Buffer：
        - 先把脏页写到 共享表空间的 Doublewrite Buffer（连续空间）。
        - 再从 Doublewrite Buffer 写到真正的数据文件。
        - 如果宕机，只要有一份完整页，就能恢复。
    + 崩溃恢复时
      - 已提交事务：
        - 如果数据页没落盘，可以用 Redo Log 重放，保证提交的修改不丢。
      - 未提交事务：
        - Redo Log 里只有 prepare，没有 commit → 回滚（用 Undo Log）。
      - 页损坏：
        - Doublewrite 机制保证总能找到一份完整页。
  ...
  - mysql的explain有什么作用？
    ...
    - 对于执行计划，参数有：
      ...
      - extra 显示的结果，这里说几个重要的参考指标：
        ...
        #image("Screenshot_20250926_105917.png")
  ...
  - 给你张表，发现查询速度很慢，你有那些解决方案
    ...
    #image("Screenshot_20250926_110147.png")
  ...
  - SELECT \* FROM t WHERE a = 1 AND b > 2 ORDER BY e, d, f; 这条语句如何加索引
    - 加aedfb的索引
    - 如果加abedf,b范围查询后，edf的索引无效要走回表
    - aedfb可以利用索引下推（在索引移动的同时判断），在按edf排序的同时判断b是否>2
    - 遇到这种情况，如果可以优化sql的话，可以将b>2换成b in （3,4,5...）
    - 主要看数据存储分布
    - 如果b的筛选性强，可能优化器也会优化成ab，edf要回表+filesort

  - semi-join优化
    - 把 IN / EXISTS / = ANY() 都统一优化为一种执行策略。
    #image("Screenshot_20251013_233348.png")
    - exists和in会被优化器优化，你写in可能优化成半连接查询低层执行类似exists
