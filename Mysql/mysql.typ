...
+ 锁篇
  ...
  - update 没加索引会锁全表？
    - 在线上执行一条 update 语句修改数据库数据的时候，where 条件没有带上索引，导致业务直接崩了
    - 为什么会发生这种的事故？
    ...
    - 如何避免这种事故的发生？
      ...
      - elete 语句必须满足以下条件能执行成功：
        - 同时使用 where 和 limit，此时 where 条件中可以没有索引列；
        - 如果 where 条件带上了索引列，但是优化器最终扫描选择的是全表，而不是索引的话，我们可以使用 force index([index_name]) 可以告诉优化器使用哪个索引，以此避免有几率锁全表带来的隐患。
  - MySQL 记录锁+间隙锁可以防止删除操作而导致的幻读吗？
    #image("Screenshot_20250826_104215.png")
    - 为什么只是查询年龄 20 岁以上行记录，而把整个表给锁住了呢？
      ...
      - 如果对 age 建立索引，事务 A 这条查询会加什么锁呢？
        ...
        - 对 age 字段建立了索引后，查询语句是索引查询，并不会全表扫描
  ...
  - MySQL 死锁了，怎么办？
    ...
    - 死锁的发生
      ...
      - #image("Screenshot_20250826_105240.png")
      ...
      - #image("Screenshot_20250826_105322.png")
    - 为什么会产生死锁？
      - 此时事务 A 在二级索引（INDEX_NAME : index_order）上加的是 X 型的 next-key 锁，锁范围是(1006, +∞\]。
      - 上加的是 X 型的 next-key 锁，锁范围是(1006, +∞\]。如果 LOCK_MODE 是 next-key 锁或者间隙锁，那么 LOCK_DATA 就表示锁的范围最右值
      - #image("Screenshot_20250826_105728.png")
      ...
      - 为什么间隙锁与间隙锁之间是兼容的？
        - 间隙锁的意义只在于阻止区间被插入，因此是可以共存的。一个事务获取的间隙锁不会阻止另一个事务获取同一个间隙范围的间隙锁，共享和排他的间隙锁是没有区别的，他们相互不冲突，且功能相同即两个事务可以同时持有包含共同间隙的间隙锁。
        - 这里的共同间隙包括两种场景：
          - 其一是两个间隙锁的间隙区间完全一样；
      ...
      - 插入意向锁是 依附于 Next-Key Lock 的辅助机制，它们互斥，从而保证插入操作受控。
      -  Insert 语句是怎么加行级锁的？
        ...
        - 什么是隐式锁？
          ...
          - 隐式锁就是在 Insert 过程中不加锁，只有在特殊情况下，才会将隐式锁转换为显示锁，这里我们列举两个场景。
            ...
            - 并发多个事务的时候，第一个事务插入的记录，并不会加锁，而是会用隐式锁保护唯一二级索引的记录。但是当第一个事务还未提交的时候，有其他事务插入了与第一个事务相同的记录，第二个事务就会被阻塞，因为此时第一事务插入的记录中的隐式锁会变为显示锁且类型是 X 型的记录锁，而第二个事务是想对该记录加上 S 型的 next-key 锁，X 型与 S 型的锁是冲突的
            -  如果 order_no 不是唯一二级索引，那么两个事务，前后执行相同的 Insert 语句，是不会发生阻塞的
            #image("Screenshot_20250826_112830.png")
            #image("Screenshot_20250826_112855.png")
  - 如何避免死锁？
  ...
  - 字节面试：加了什么锁，导致死锁的？
    #image("Screenshot_20250826_152652.png")
    - #image("Screenshot_20250826_152731.png")
    -  为什么会发生死锁？
      - #image("Screenshot_20250826_152825.png")
      - #image("Screenshot_20250826_152845.png")
      - #image("Screenshot_20250826_152917.png")
      - #image("Screenshot_20250826_152932.png")
      - 最后回答，为什么会发生死锁？
      ...
      #image("Screenshot_20250826_153058.png")
      - 如果只有事务a或b的情况
      #image("Screenshot_20250826_153438.png")
+ 日志篇
  - MySQL 日志：undo log、redo log、binlog 有什么用？
    - 更新语句update流程
    #image("Screenshot_20250827_104812.png")
    ...
    - 为什么需要 undo log？
      ...
      #image("Screenshot_20250827_105553.png")
      ...
      - 一条记录的每一次更新操作产生的 undo log 格式都有一个 roll_pointer 指针和一个 trx_id 事务id：
        ...
        #image("Screenshot_20250827_110235.png")
      - 另外，undo log 还有一个作用，通过 ReadView + undo log 实现 MVCC
      - 对于「读提交」和「可重复读」隔离级别的事务来说，它们的快照读（普通 select 语句）是通过 Read View + undo log 来实现的，它们的区别在于创建 Read View 的时机不同：
        - 「读提交」隔离级别是在每个 select 都会生成一个新的 Read View，也意味着，事务期间的多次读取同一条数据，前后两次读的数据可能会出现不一致，因为可能这期间另外一个事务修改了该记录，并提交了事务。
        - 「可重复读」隔离级别是启动事务时生成一个 Read View，然后整个事务期间都在用这个 Read View，这样就保证了在事务期间读到的数据都是事务启动前的记录。
      - 这两个隔离级别实现是通过「事务的 Read View 里的字段」和「记录中的两个隐藏列（trx_id 和 roll_pointer）」的比对，如果不满足可见行，就会顺着 undo log 版本链里找到满足其可见性的记录，从而控制并发事务访问同一个记录时的行为，这就叫 MVCC
    - 因此，undo log 两大作用：
      - 实现事务回滚，保障事务的原子性。
   ...
   - 为什么需要 Buffer Pool？
     ...
     - 有了 Buffer Poo 后：
       ...
       - 客户端就会直接读取 Buffer Pool 中的数据，否则再去磁盘中读取。然后将其页设置为脏页（该页的内存数据和磁盘上的数据已经不一致），为了减少磁盘I/O，不会立即将脏页写入磁盘，后续由后台线程选择一个合适的时机将脏页写入到磁盘。
     - Buffer Pool 缓存什么？
       ...
       - Buffer Pool 除了缓存「索引页」和「数据页」，还包括了 Undo 页，插入缓存、自适应哈希索引、锁信息等等。
         ...
         - 查询一条记录，就只需要缓冲一条记录吗？
           ...
           #image("Screenshot_20250827_111853.png")
    - 为什么需要 redo log ？
      - Buffer Pool 是提高了读写效率没错，但是问题来了，Buffer Pool 是基于内存的，而内存总是不可靠，万一断电重启，还没来得及落盘的脏页数据就会丢失。
      - 为了防止断电导致数据丢失的问题，当有一条记录需要更新的时候，InnoDB 引擎就会先更新内存（同时标记为脏页），然后将本次对这个页的修改以 redo log 的形式记录下来，这个时候更新就算完成了。
      - 后续，InnoDB 引擎会在适当的时候，由后台线程将缓后续，InnoDB 引擎会在适当的时候，由后台线程将缓后续，InnoDB 引擎会在适当的时候，由后台线程将缓
      ...
      #image("Screenshot_20250827_155131.png")
      ...
      - redo log 什么时候刷盘？
        - redo log 什么时候刷盘？
          - MySQL 正常关闭时；
          - 当 redo log buffer 中记录的写入量大于redo log buffer 内存空间的一半时，会触发落盘；
        ...
        - innodb_flush_log_at_trx_commit 参数控制的是什么？
          ...
          - 除此之外，InnoDB 还提供了另外两种策略，由参数 innodb_flush_log_at_trx_commit 参数控制，可取的值有：0、1、2，默认值为 1，这三个值分别代表的策略如下：
            - 当设置该参数为 0 时，表示每次事务提交时 ，还是将 redo log留在 redo log buffer 中 ，该模式下在事务提交时不会主动触发写入磁盘的操作。
            - 当设置该参数为 1 时，表示每次事务提交时，都将缓存在redo log buffer 里的 redo log 直接持久化到磁盘，这样可以保证 MySQL 异常重启之后数据不会丢失。
        ...
        -  redo log 文件写满了怎么办？
          ...
          - 所以 InnoDB 存储引擎会先写 ib_logfile0 文件，当ib_logfile0 文件被写满的时候，会切换至 ib_logfile1 文件，当 ib_logfile1 文件也被写满时，会切换回 ib_logfile0 文件。
          ...
          #image("Screenshot_20250827_161116.png")
    ...
    - 为什么需要 binlog ？
      ...
      - redo log 和 binlog 有什么区别？
        ...
        + 文件格式不同：
          - binlog 有 3 种格式类型，分别是 STATEMENT（默认格式）、ROW、 MIXED，区别如下： 
            ...
            - ROW：记录行数据最终被修改成什么样了（这种格式的日志，就不能称为逻辑日志了），不会出现 STATEMENT 下动态函数的问题。但 ROW 的缺点是每行数据的变化结果都会被记录，比如执行批量update 语句，更新多少行数据就会产生多少条记录，使 binlog 文件过大，而在 STATEMENT 格式下只会记录一个 update 语句而已；
            - MIXED：包含了 STATEMENT 和 ROW 模式，它会根据不同的情况自动使用 ROW 模式和 STATEMENT 模式；
          - redo log 是物理日志，记录的是在某个数据页做了什么修改，比如对 XXX 表空间中的 YYY 数据页 ZZZ 偏移量的地方做了AAA 更新；
          - 物理日志和逻辑日志
          #image("Screenshot_20250827_162513.png")
        + 写入方式不同：
      ...
      - 如果不小心整个数据库的数据被删除了，能使用 redo log 文件恢复数据吗？
        ...
        - binlog没有crashsafe能力，所以需redolog配合
    - 主从复制是怎么实现？
    ...
    -  binlog 什么时候刷盘？
      ...
      - 一个事务的 binlog 是不能被拆开的，因此无论这个事务有多大比如有很多条语句），也要保证一次性写入。这是因为有一个线程只能同时有一个事务在执行的设定，所以每当执行一个 begin/start transaction 的时候，就会默认提交上一个事务，这样如果一个事务的 binlog 被拆开的时候，在备库执行就会被当做多个事务分段自行，这样破坏了原子性，是有问题的。
      - MySQL 给每个线程分配了一片内存用于缓冲 binlog ，该内存叫 binlog cache，参数 binlog_cache_size 用于控制单个线程内 binlog cache 所占内存的大小。如果超过了这个参数规定的大小，就要暂存到磁盘。
      - 什么时候 binlog cache 会写到 binlog 文件？
        - 在事务提交的时候，执行器把 binlog cache 里的完整事务写入到 binlog 文件中，并清空 binlog cache。
        - 虽然每个线程有自己 binlog cache，但是最终都写到同一个 binlog 文件：
        #image("Screenshot_20250827_165106.png")
      ...
      #image("Screenshot_20250827_165527.png")
      -   undo页面修改意味着旧的数据行被修改，所以需要记录对应的redolog
  - 为什么需要两阶段提交？
    - 事务提交后，redo log 和 binlog 都要持久化到磁盘，但是这两个是独立的逻辑，可能出现半成功的状态，这样就造成两份日志之间的逻辑不一致。
    - 举个例子，假设 id = 1 这行数据的字段 name 的值原本是 'jay'，然后执行 UPDATE t_user SET name = 'xiaolin' WHERE id = 1; 如果在持久化 redo log 和 binlog 两个日志的过程中，出现了半成功状态，那么就有两种情况：
      - 如果在将 redo log 刷入到磁盘之后， MySQL 突然宕机了，而 binlog 还没有来得及写入。
        - MySQL 重启后，通过 redo log 能将 Buffer PoolPool 中 id = 1 这行数据的 name 字段恢复到新值 xiaolin，但是 binlog 里面没有记录这条更新语句，在主从架构中，binlog 会被复制到从库，由于 binlog 丢失了这条更新语句，从库的这一行 name 字段是旧值 jay，与主库的值不一致性；
      - 如果在将 binlog 刷入到磁盘之后， MySQL 突然宕机了，而 redo log 还没有来得及写入。由于 redo log 还没写，崩溃恢复以后这个事务无效，所以 id = 1 这行数据的 name 字段还是旧值 jay，而 binlog 里面记录了这条更新语句，在主从架构中，binlog 会被复制到从库，从库执行了这条更新语句，那么这一行 name 字段是新值 xiaolin，与主库的值不一致性；

    - 可以看到，在持久化 redo log 和 binlog 这两份日志的时候，如果出现半成功的状态，就会造成主从环境的数据不一致性。这是因为 redo log 影响主库的数据，binlog 影响从库的数据，所以 redo log 和 binlog 必须保持一致才能保证主从数据一致。
    - MySQL 为了避免出现两份日志之间的逻辑不一致的问题，使用了「两阶段提交」来解决，两阶段提交其实是分布式事务一致性协议，它可以保证多个逻辑操作要不全部成功，要不全部失败，不会出现半成功的状态。
    - 两阶段提交把单个事务的提交拆分成了 2 个阶段，分分别是「准备（Prepare）阶段」和「提交（Commit）阶段」，每个阶段都由协调者（Coordinator）和参与者（Participant）共同完成。注意，不要把提交（Commit）阶段和 commit 语句混淆了，commit 语句执行的时候，会包含提交（Commit）阶段。
    #image("Screenshot_20250827_204539.png")
    -  两阶段提交的过程是怎样的？
      ...
      #image("Screenshot_20250827_204708.png")
      - 从图中可看出，事务的提交过程有两个阶段，就是将 redo log 的写入拆成了两个步骤：prepare 和 commit，中间再穿插写入binlog，具体如下：
        - prepare 阶段：将 XID（内部 XA 事务的 ID） 写入到 redo log，同时将 redo log 对应的事务状态设置为 prepare， 然后将 redo log 持久化到磁盘（innodb_flush_log_at_trx_commit = 1 的作用）；
        - commit 阶段：把 XID 写入到 binlog，然后将 binlog 持久化到磁盘 （sync_binlog = 1 的作用），接着调用引擎的提交事务接口，将 redo log 状态设置为 commit，此时该状态并不需要持久化到磁盘，只需要 write 到文件系统的 page cache 中就够了，因为只要 binlog 写磁盘成功，就算 redo log 的状态    还是 prepare 也没有关系，一样会被认为事务已经执行成功；
    - 异常重启会出现什么现象？
  ...
  - 事务没提交的时候，redo log 会被持久化到磁盘吗？
    ...
    - 所以， redo log 可以在事务没提交之前持久化到磁盘，，但是 binlog 必须在事务提交之后（也就是commit之后），才可以持久化到磁盘。
    #image("Screenshot_20250827_210659.png")
  - 两阶段提交有什么问题？
    ...
    - 为什么两阶段提交的磁盘 I/O 次数会很高？
      - binlog 和 redo log 在内存中都对应的缓存空间，binlog 会缓存在 binlog cache，redo log 会缓存在 redo log buffer，它们持久化到磁盘的时机分别由下面这两个参数控制。一般我们为了避免日志丢失的风险，会将这两个参数设置为 1：
        ...
        - 当 innodb_flush_log_at_trx_commit = 1 时，表示每次事务提交时，都将缓存在存在 redo log buffer 里的 redo log 直接持久化到磁盘；
      - 那么在每个事务提交过程中， 都会至少调用 2 次刷盘操作
    - 为什么锁竞争激烈？
      - 在早期的 MySQL 版本中，通过使用 prepare_commit_mutex锁来保证事务提交的顺序，在一个事务获取到锁时才能进入 prepare 阶段，一直到 commit 阶段结束才能释放锁，下个事务才可以继续进行 prepare 操作。
    - 组提交
      ...
      -   flush 阶段
        - #image("Screenshot_20250827_212055.png")
        - #image("Screenshot_20250827_212110.png")
        - #image("Screenshot_20250827_212133.png")  
      - sync 阶段
        - #image("Screenshot_20250827_212159.png")
        - #image("Screenshot_20250827_212208.png")
        - #image("Screenshot_20250827_212221.png")
      ...
      - commit 阶段
        - #image("Screenshot_20250827_212349.png")
    ...
    -  MySQL 磁盘 I/O 很高，有什么优化的方法？
        ...
        - 将 innodb_flush_log_at_trx_commit 设置为 2。表示每次事务提交时，都只是缓存在 redo log buffer 里的 redo log 写到 redo log 文件，注意写入到「 redo log 文件」并不意味着写入到了磁盘，因为操作系统的文件系统中有个 Page Cache，专门用来缓存文件数据的，所以写入「 redo log文件」意味着写入到了操作系统的文件缓存，然后交由操作系统控制持久化到磁盘的时机。但是这样做的风险是，主机    掉电的时候会丢数据。
    #image("Screenshot_20250827_212806.png")
+ 内存篇
  - 为什么要有 Buffer Pool？
  ...
  #image("Screenshot_20250828_105651.png")
  ...
  - 如何管理 Buffer Pool？
    ...
    #image("Screenshot_20250828_105926.png")
  ...
  -  如何提高缓存命中率？
    ...
    - 简单的 LRU 算法的实现思路是这样的：
      ...
      #image("Screenshot_20250828_110429.png")
    - 这里我们可以知道，Buffer Pool 里有三种页和链表来管理数据。
    ...
    - 怎么解决预读失效而导致缓存命中率降低的问题？
      ...
      - MySQL 是这样做的，它改进了 LRU 算法，将 LRU 划分了 2 个区域：old 区域 和 young 区域。
        ...
        #image("Screenshot_20250828_110740.png")
        - old 区域占整个 LRU 链表长度的比例可以通过innodb_old_blocks_pct 参数来设置，默认是 37，代表整个 LRU 链表中 young 区域与 old 区域比例是 63:37。
        - 划分这两个区域后，预读的页就只需要加入到 old 区域的头部，当页被真正访问的时候，才将页插入 young 区域的头部。如果预读的页一直没有被访问，就会从 old 区域移除，这样就不会影响 young 区域中的热点数据。
      - #image("Screenshot_20250828_110827.png")
      ...
      - 什么是 Buffer Pool 污染？
        ...
        - 可能这个查询出来的结果就几条记录，但是由于这条语句会发生索引失效，所以这个查询过程是全表扫描的，接着会发生如下的过程：
          ...
          - 接下来拿行记录的 name 字段和字符串 xiaolin 进行模糊匹配，如果符合条件，就加入到结果集里；如此往复，直到扫描完表中的所有记录。
        - 经过这一番折腾，原本 young 区域的热点数据都会被替换掉。
        #image("Screenshot_20250828_111110.png")
