  ...
  - 点对点
    ...
    - 可靠传输
      ...
      - 选择重传
        ...
        - 如果在传输过程中有传输失败的分组，发送方只重传那些传输失败的分组，并且在接收到ack时，窗口不移动
        - 如果发送方传出1,2,3号分组，2号分组丢失，接收方接收到1,3号分组，发送ack1,ack3，发送方收到ack1,ack3后只重传2号分组，并且会标记1号，3号分组已经被确认，避免超时重传，丢失的2号分组等待超时重传
        - 直到发送方和传送方都被确认正确接收，则窗口移动
    - 点对点协议PPP
      - 帧的首部
        - 帧格式
          - 标志
      ...
      - 透明传输
        - 字节填充法
        #image("Screenshot_20250916_110301.png")
        - 比特填充法
        #image("Screenshot_20250916_110415.png")
        - 工作状态
        #image("Screenshot_20250916_110600.png")
  - 媒体接入控制
    - 静态划分信道
      - 信道复用
        - 一条物理线同时传多路用户的信号
        ...
        - 码分复用
          ...
          - 使用CMDA的每一个站都会分配一个码片序列
            - 如果一个站发送1,就发送他的m bit码片序列
            - 如果发送0,就发送m bit码片序列的反码
          - 码片序列的挑选原则
            - 分配每个站的码片序列必须各不相同
            - 分配每个站的码片序列必须互相正交
  - 随机接入
    - CSMA/CA协议的工作原理
    #image("Screenshot_20250925_114758.png")
    - CSMA/CA协议的退避算法
    #image("Screenshot_20250925_115243.png")
    - CSMA/CA协议的信道预约和虚拟载波监听
    #image("Screenshot_20250925_115619.png")
