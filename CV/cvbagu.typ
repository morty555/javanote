...
= hr面 
  - 自我介绍
  - 讲一下你在大学里面做过的最有成就感的事
    - 第一个项目从0到1搭建，自己统筹管理+需求分析
  - 未来打算考公/考研吗？
    - NO
  - 说一下你的职业规划
    - 目前，通过实习了解大厂的业务流程，学习新技术，接触到真实大型项目
    - 3-5年，成为某一技术领域的专家，承担更多责任
  - 你现在手上有几个offer？薪资待遇如何？
    - 十一月有面过几个小厂给了OFFER但是因为时间不合适都拒掉了，因为腾讯在流程中还是比较想去腾讯
  - 你除了投递我们公司，还有哪些公司呢？还有哪些面试？
    - 目前偏向于在广东工作，投递的公司不是很多，大厂其实就腾讯一个，其他都是一些中小型公司
    - 腾讯约面之后就一直在准备腾讯的面试，这段时间也没有新投什么岗位了
  - 选择企业最看重的三个点
    - 平台（能接触到什么资源和业务）和 培养机制（能从企业获得什么成长）
    - 企业文化（价值观和工作氛围）
    - 福利待遇（薪资、晋升、工作环境等）
  - 拷问父母工作
  - 如何看待996
    - 我不排斥偶尔的加班，我平时的学习强度也接近996，但我认为持续的 996 不是最佳方式，正常工作时间内我觉得是可以完成工作内容，就不需要加班来处理，长时间的工作会带来效率的降低，更重要的是合理的计划、明确的目标和良好的团队协作，这样才能把效率最大化。
  - 对应聘公司产品的了解和满意度，如果让你改进公司产品你会怎么做
    - 说说微信，QQ，腾讯会议，腾讯地图和游戏体系
    - 改进产品这里可以提一下腾讯会议的不同系统的兼容性
  - 对ai(新技术)的了解
    - 工程应用方面的了解
      - 用过 OpenAI、阿里 Qwen、DeepSeek API
      - 使用向量数据库（如 Milvus、FAISS）做 RAG
    - 对风险和最佳实践有认知
      - 限制模型幻觉
      - 数据隐私管理
      - 提示词优化
  ...
  - 为什么选择这个岗
    - 不局限自己走一个方向
  - 形容下你自己
    - 责任心：我对团队和工作有责任感，不太会把问题甩给别人，而是愿意主动承担并推动事情落地。
    - 我的学习能力比较强，遇到不会的东西，我会主动查资料、动手尝试，通常能很快进入状态。
    - 做事踏实，自觉，容易沟通
  - 为什么选我们
    - 腾讯的平台技术与产品环境成熟，能让我学习到更前沿的技术和业务
    - 腾讯的工作氛围很不错  
    - 在一个大用户量基础的产品上做工作更有自豪感
  - 腾讯的前沿科技有什么？ 
    - “混元 Turbo”是腾讯自研大模型服务（基于 MoE 架构），据称训练效率提升、推理成本下降，并作为基础模型支撑腾讯云的新一代 AI 服务。2025 年，腾讯通过“混元 + 开源模型”的组合方案，将大模型能力推广到金融、医疗、教育、政务、媒体、文旅等 30+ 行业。
    - 作为腾讯云边缘/安全/加速平台，EdgeOne 新集成了大模型能力（MCP Server + LLM），并通过 EdgeOne Pages 提供将网站从“开发到上线”时间从“1 天”压缩到“1 分钟”的工具。这对需要快速上线、响应全球用户的互联网服务尤为重要。
    - 随着混元大模型能力提升，腾讯推出 CodeBuddy，用于帮助开发者编写代码。官方称可减少约 40% 编码时间、提高约 16% 的研发效率。它代表腾讯在 “AI 助手 + 开发工具链” 方向的加速布局。
    - ADP 3.0 是智能 agent（AI 代理／AI agent）开发平台，结合 Agent Runtime，为企业提供构建自主智能 agent 的完整基础设施，对客服、营销、库存管理、研究等多个场景提供自动化
  - 在项目中遇到最大的困难是什么，如何解决的
    - 刚接触项目时技术的不成熟，需求不清晰，纤维领域知识陌生，很多功能都没有一些完善的解决方案
    - 一直在学习新技术，和老师，组内人员积极沟通，对接需求
  - 当初为什么会选择计算机专业
    - 自己小时候比较喜欢看科幻电影，对黑客，赛博朋克感兴趣，觉得计算机是未来生产力
  - 讲一个你的项目
  - 未来希望发展的技术方向
    - 如果目前这份实习能够通过，就肯定是深耕团队业务需要的技术方向
    - 如果没有的话就还是考虑会学习后端技术栈，大数据等方向
  - 有压力的经历
    - 在这个项目的一次对接功能中，需求方临时变更了需求，导致我需要在很短的时间内完成一个之前没有接触过的功能模块，并且当时手上有一个正在改的功能还未完善，手头上这个功能也会影响展示的效果，时间很紧迫
    - 以新需求为主，手头上的功能找到一个折中的方案，先保证不影响新需求的完成，然后完成新需求后再完善之前的功能
  - 怎么和产品同事沟通
    - 理清楚需求背景、核心目标、优先级
    - 如果需求复杂或周期紧，我会从技术角度说明成本、实现难度和潜在风险
    - 做需求时我会保持进度透明，有问题及时同步，避免临上线前才翻车。
  - 本科的校园经历
    - 大一大二心理委员，参与院的一些心理活动的组织和后勤支持
    - 参与电竞社社团比赛的一些策划和组织    
  - 假如说可以立马解决你一个缺点你选择哪个
    - 不会拒绝别人，我想改善自己有时太容易答应别人请求的问题。虽然我愿意帮忙，但这偶尔会让我分心。
    - 做决定前思考太多，希望自己果断一点
  - 你在生活中有没有特别佩服的人
    - 马斯克
    - 长期主义：做别人不敢做、看得更远的事。“他做 SpaceX、做特斯拉，并不是为了短期收益，而是在解决人类更长期的问题。这种在巨大不确定性下依然坚持的勇气，是我非常佩服的。”
    - 工程思维：问题拆解能力非常强。马斯克最厉害的是他的第一性原理：把问题拆到最底层，再重新找最优解。
    - 执行力：说到做到，即使困难巨大也不放弃。SpaceX连续三次失败后公司接近破产仍坚持第四次试射，这种承压能力和执行力，是我自己也在学习的。
  - 组内你和另一个人的想法只能采取一个
    - 以达成目标为导向
...
- JAVAAOP原理
  ...
  - 核心概念
    - 切面（Aspect）：封装横切逻辑的模块
    - 连接点（Join Point）：程序执行的点，比如方法调用。
    - 通知（Advice）：切面在特定连接点要执行的逻辑，例如 before、after、around。
    - 切入点（Pointcut）：匹配连接点的表达式，决定哪些方法会被增强。
    - 代理对象（Proxy）：AOP 实际操作的核心，代理目标对象，将增强逻辑织入方法调用。
  - Spring AOP 里主要有两种代理方式：
    - JDK动态代理 
      - 使用 java.lang.reflect.Proxy 生成目标接口的代理类。
      - 代理类实现目标接口，内部通过 InvocationHandler 拦截方法调用。
      - 在 invoke 方法中可以执行切面逻辑（增强）。
      - 只能代理接口，如果目标类没有接口无法使用。
    - CGLIB
      - 基于 字节码生成技术（通过 Enhancer），生成目标类的子类。
      - 子类重写目标类方法，在方法中插入增强逻辑。
      - 可以代理 没有接口的类。
      - 生成代理类比 JDK 代理开销大。不能代理 final 类和 final 方法，因为无法重写。可能增加类加载和内存开销。
...
- redis 过期时间
  - 三十分钟到一个小时，太长对内存占用大，最好根据用户对话条数来调整，多轮对话就延长过期时间
...
- Redis与RAG结合
  ...
  - REDIS短期记忆保持当前会话连续性，RAG长期记忆提供用户历史偏好、知识、重要信息，增强回答准确性
...
- 用bitmap存储签到状态
  - bitmap 实际是 连续的字节数组，位运算强大，连续签到计算方便
  - 如何存储的：每个USER设计一个BITMAP,按月存储
  - 签到操作：SETBIT sign:1001:202512 12 1
  - 查询某天是否签到：GETBIT sign:1001:202512 12
  - 统计当月总签到天数：BITCOUNT sign:1001:202512

- bitmap和直接用char数组区别
  - 位运算效率快
  - 可做统计
  - bitmap多机下数据共享
  - Java 自带的 BitSet 就是 bitmap 的实现

- 按用户存bitmap和按天数存bitmap的区别
  - 空间占用相似
  - 单用户查询的话使用按用户存bitmap，直接 GETBIT（某天）即可，而按天需要取当天 bitmap，再定位用户位
  - 统计单用户连续签到，按用户存的可以bitmap 连续位操作即可，而按天存需跨天取不同 bitmap 再组合连续签到
  - 统计单天活跃用户数，按用户需要需遍历所有用户 bitmap 对当天位做统计，而按天快：直接 BITCOUNT 当天 bitmap
  - 跨用户统计，按用户存需要取每个用户 bitmap 做位操作，按天直接 BITOP AND/OR 多天 bitmap
  - 如何选取看场景：关注用户连续签到、奖励发放、单用户统计用按用户存，关注每日活跃统计、全局投票、打卡人数统计按天存

- 我这里不需要统计某天的总签到人数，而更在意用户每天连续签到的场景，所以使用按用户来区分bitmap
- 而且由于内存占用和功能只看每月签到数等，每月 31 天 → bitmap 长度 31 位
- 如果是只关注当月签到奖励的简单系统，换月后bitmap可以删除节省内存
- 如果是需要统计连续签到跨月，建议保留历史bitmap，保存到数据库中，清空 Redis 中的旧 bitmap，释放内存
